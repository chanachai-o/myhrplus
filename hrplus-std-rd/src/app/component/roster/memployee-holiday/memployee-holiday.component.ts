...
import { NgbActiveModal, NgbDatepickerModule, NgbModal, NgbModalRef, NgbModule, NgbPaginationModule } from '@ng-bootstrap/ng-bootstrap';
...
@Component({
  standalone: true,
  imports: [CommonModule, TranslateModule, FormsModule, ScrollingModule, NgbModule, NgbDatepickerModule, NgbPaginationModule,],
  selector: 'app-memployee-holiday',
  templateUrl: './memployee-holiday.component.html',
  styleUrls: ['./memployee-holiday.component.scss']
})
export class MemployeeHolidayComponent implements OnInit {
  search = ""
  empList: any[] = []
  page = 1
  pageSize = 10
  ...
  getEmpWorkAreaLists() {
    ...
    this.employeeService.getEmpSubordinates().pipe(map(x => x.map(y => new KerryEmployeeModel(y, this.translateService)))).subscribe(response => {
...
@Component({
  selector: 'app-memployee-holiday-modal',
  templateUrl: './memployee-holiday-modal.component.html',
  styleUrls: ['./memployee-holiday.component.scss'],
  imports: [
    CommonModule,
    FormsModule,
...
export class MemployeeHolidayModalComponent implements OnInit {
  @Input() publicHolidaysList: KerryPublicHolidayModel[] = []
  @Input() publicHolidaysFilter: KerryPublicHolidayModel[] = []
  page = 1
  pageSize = 10
  search = ""
  ...
  ngOnInit(): void {
    this.publicHolidaysFilter = this.publicHolidaysList
  }

  searchData(text: string) {
    text = text.toLowerCase()
    this.publicHolidaysFilter = text ? this.publicHolidaysList.filter(x => {
      if (x.tdesc.toLowerCase().includes(text) ||
        x.edesc.toLowerCase().includes(text)) {
        return x
      }
    }) : this.publicHolidaysList
    this.page = 1
  }
