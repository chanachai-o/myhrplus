...
import { NgbModal, NgbPaginationModule, NgbPaginationPages } from '@ng-bootstrap/ng-bootstrap';
...
@Component({
    standalone: true,
  imports: [CommonModule, TranslateModule, FormsModule, ScrollingModule, NgbPaginationModule],
    selector: 'app-rgm-approved-by-store',
    templateUrl: './rgm-approved-by-store.component.html',
    styleUrls: ['./rgm-approved-by-store.component.scss']
  })
  export class RgmApprovedByStoreComponent implements OnInit {
    ...
    searchData() {
        ...
        this.timeService.getTimeCurrentList(body).then(response => {
            ...
            let checkListTime = this.timecurrentList.find(x => x.ttimeCurrentSum.ttimeCurrentList.find(i => i.rgmApprove == true))
            this.checkApproveAll = checkListTime?true:false
    ...
    saveLeaderManagerApproveAll(){
        ...
        this.timeService.postLeaderApproveAll(this.timecurrentListApproveAll).then(result => {
        ...
        // อันเดิมส่ง model ค้นหาข้อมูลตามวัน

        // let monthStart = this.selectMonth == 1 ? 12 : this.selectMonth - 1
        // let yearStart = this.selectMonth == 1 ? this.selectYear - 1 : this.selectYear
        // let startDate = yearStart + '-' + ('0' + monthStart).slice(-2) + '-' + (this.dateType == 1 ? '16' : '25')
        // const modalData = {
        //     workareaId: [this.selectWorkArea.workareaId],
        //     period: startDate,
        //     status: "6"
        //   }
        // this.timeService.postLeaderManagerApproveAll(modalData).then(result => {
        //     this.msg = result.message
        //     this.ngbModal.open(this.alertModal,{
        //         centered: true,
        //         backdrop: 'static'
    ...
    exportexcel(): void
    {
    this.fileName = 'Leader Approved Workarea '+ (this.currentDate.getDate()+"-"+(this.currentDate.getMonth()+1+'-'+this.currentDate.getFullYear()))+'.xlsx'
