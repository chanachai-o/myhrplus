...
@Component({
  ...
  selector: 'app-set-ot',
  templateUrl: './set-ot.component.html',
  styleUrls: ['./set-ot.component.scss']
})
export class SetOTComponent implements OnInit {
  ...
  deleteSetOTList() {
    if (this.setOTList.every(x => x.checkbox == false)) {
      this.openAlertModal(this.translateService.currentLang == 'th' ? 'กรุณาเลือกข้อมูลที่ต้องการลบ' : 'Please select data.')
    } else {
      this.setOTList = this.setOTList.filter(x => x.checkbox == false)
    }
  }
  setOTCheckAll(value: any) {
    this.setOTList = this.setOTList.map(x => {
  ...
  checkSetOT() {
    if (this.setOTList.every(x => x.checkbox == false)) {
      ...
      modalRef.result.then(result => {
        this.activeModal.close(this.setOTList.filter(x => x.checkbox).map((x, i) => {
          return {
            lineNo: i + 1,
  ...
  openSetOTEditModal(modalName: any, index?: string) {
    if (index?.toString()) {
      const dateStart = this.setOTList[parseInt(index)].dateStart.split("-").map(x => parseInt(x))
      const dateEnd = this.setOTList[parseInt(index)].dateEnd.split("-").map(x => parseInt(x))
      const employee = this.employeeList[this.employeeList.findIndex(x => x.id == this.setOTList[parseInt(index)].employee.id)]
      this.setOT = { ...this.setOTList[parseInt(index)], employee: employee, dateStart: new NgbDate(dateStart[0], dateStart[1], dateStart[2]), dateEnd: new NgbDate(dateEnd[0], dateEnd[1], dateEnd[2]) }
    } else {
      this.setOT = {
    ...
    this.setOTEditModal = this.ngbModal.open(modalName, {
      centered: true,
      backdrop: 'static',
      size: 'lg'
    })
    this.setOTEditModal.result.then(result => {
      const timeStart = this.setOT.timeStart.split(":")
      const timeEnd = this.setOT.timeEnd.split(":")
      const dateStart = new Date(this.setOT.dateStart.year, this.setOT.dateStart.month - 1, this.setOT.dateStart.day, parseInt(timeStart[0]), parseInt(timeStart[1])).getTime()
      const dateEnd = new Date(this.setOT.dateEnd.year, this.setOT.dateEnd.month - 1, this.setOT.dateEnd.day, parseInt(timeEnd[0]), parseInt(timeEnd[1])).getTime()
      ...
      if (index?.toString()) {
        this.setOTList[parseInt(index)] = {
          ...this.setOT,
          checkbox: false,
          timeTotal: hour + ":" + min,
          dateStart: this.formatYYYY_MM_DD(new Date(this.setOT.dateStart.year + "-" + this.setOT.dateStart.month + "-" + this.setOT.dateStart.day)),
          dateEnd: this.formatYYYY_MM_DD(new Date(this.setOT.dateEnd.year + "-" + this.setOT.dateEnd.month + "-" + this.setOT.dateEnd.day)),
        }
      } else {
        this.setOTList.push({
          ...this.setOT,
  ...
  checkSetOTEditModal() {
    ...
    if (typeof this.setOT.dateStart == "object" && typeof this.setOT.dateEnd == "object") {
      if (this.setOT.dateStart?.year && this.setOT.dateStart?.month && this.setOT.dateStart?.day && this.setOT.dateEnd?.year && this.setOT.dateEnd?.month && this.setOT.dateEnd?.day) {
        if (this.setOT.timeStart != "" &&
          this.setOT.timeEnd != "" &&
          this.setOT.reason.getName() != "" &&
          this.setOT.workarea.getName() != "" &&
          this.setOT.costCenter.getName() != "" &&
          this.setOT.time0.getName() != "" &&
          this.setOT.remark != "") {
          const dateStart = this.formatYYYY_MM_DD(new Date(this.setOT.dateStart.year + "-" + this.setOT.dateStart.month + "-" + this.setOT.dateStart.day))
          const dateEnd = this.formatYYYY_MM_DD(new Date(this.setOT.dateEnd.year + "-" + this.setOT.dateEnd.month + "-" + this.setOT.dateEnd.day))
          if (parseInt(dateStart.split("-").join("")) > parseInt(dateEnd.split("-").join(""))) {
            ...
            const timeStart = this.setOT.timeStart.replace(":", ".")
            const timeEnd = this.setOT.timeEnd.replace(":", ".")
  ...
  openReasonModal() {
    ...
    modalRef.result.then(result => {
      this.setOT.reason = new ReasonModel(result, this.translateService)
      this.cdr.markForCheck()
    }, reason => {
      this.cdr.markForCheck()
    })
  ...
  modelReasonChange(value: string) {
    ...
    if (reason) {
      this.setOT.reason = new ReasonModel(reason, this.translateService)
    } else {
      this.setOT.reason = new ReasonModel({ reasonChangeId: value })
    }
    this.cdr.markForCheck()
  }
  ...
  openWorkareaModal() {
    ...
    modalRef.result.then(result => {
      this.setOT.workarea = new WorkAreaModel(result, this.translateService)
      this.cdr.markForCheck()
    }, reason => {
      this.cdr.markForCheck()
    })
  ...
  modelWorkareaChange(value: string) {
    ...
    if (workarea) {
      this.setOT.workarea = new WorkAreaModel(workarea, this.translateService)
    } else {
      this.setOT.workarea = new WorkAreaModel({ workareaId: value })
    }
    this.cdr.markForCheck()
  }
  ...
  openCostCenterModal() {
    ...
    modalRef.result.then(result => {
      this.setOT.costCenter = new KerryCostCenterModel(result, this.translateService)
      this.cdr.markForCheck()
    }, reason => {
      this.cdr.markForCheck()
    })
  ...
  modelCostCenterChange(value: string) {
    ...
    if (costCenter) {
      this.setOT.costCenter = new KerryCostCenterModel(costCenter, this.translateService)
    } else {
      this.setOT.costCenter = new KerryCostCenterModel({ costCenterId: value })
    }
    this.cdr.markForCheck()
  }
  ...
  openTime0Modal() {
    ...
    modalRef.result.then(result => {
      this.setOT.time0 = new KerryTime0Model(result, this.translateService)
      this.cdr.markForCheck()
    }, reason => {
      this.cdr.markForCheck()
    })
  ...
  modelTime0Change(value: string) {
    ...
    if (time0) {
      this.setOT.time0 = new KerryTime0Model(time0, this.translateService)
    } else {
      this.setOT.time0 = new KerryTime0Model({ time0id: value })
    }
    this.cdr.markForCheck()
  }
