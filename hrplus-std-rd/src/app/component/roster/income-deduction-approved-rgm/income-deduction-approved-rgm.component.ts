...
@Component({
    ...
    selector: 'app-income-deduction-approved-rgm',
    templateUrl: './income-deduction-approved-rgm.component.html',
    styleUrls: ['./income-deduction-approved-rgm.component.scss']
})
export class IncomeDeductionApprovedRgmComponent implements OnInit {
    monthList = [
    ...
    searchData() {
        ...
        this.memplService.getAmtStore(body).then(response => {
    ...
    onSave() {
        ...
        modalRef.result.then(result => {
            this.memplService.onSaveLeader(modelBody).then(response => {
    ...
    onApprove() {
        ...
        modalRef.componentInstance.message = this.translateService.instant('Do you want to save data ?');
        modalRef.result.then(result => {
            this.memplService.onApproveLeader(this.selectWorkArea.workareaId, modelBody).then(response => {
                this.openAlertModal(response.message)
                this.slectWorkArea(this.selectWorkArea)
                this.cdr.markForCheck()
            }, error => {
    ...
    searchFilter(type: string, value: string) {
        if (type === 'workarea') {
            this.workareaListShow = this.workareaList.filter(x =>
                (x.workareaId?.toLowerCase().includes(value.toLowerCase())) ||
                (x.tdesc?.toLowerCase().includes(value.toLowerCase())) ||
                (x.edesc?.toLowerCase().includes(value.toLowerCase()))
            );
        }
    }
