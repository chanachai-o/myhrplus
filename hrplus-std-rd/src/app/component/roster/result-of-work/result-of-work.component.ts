...
@Component({
    standalone: true,
  imports: [CommonModule, TranslateModule, FormsModule, ScrollingModule],
    templateUrl: './result-of-work.component.html',
    styleUrls: ['./result-of-work.component.scss']
})


export class ResultOfWorkComponent implements OnInit {
    ...
    getRoleWorkarea() {
        this.workareaService.getRoleWorakarea().then(response => {
    ...
    public onUploadFileExcel() {
        ...
        if (this.newFile) {
            ...
            this.timeService.uploadWorkingTime(body, ('0' + (this.selectMonth)).slice(-2), this.selectYear.toString(), this.selectWorkArea.workareaId).subscribe((result) => {
    ...
    approveStatus(type: string) {
        ...
        if (type == 'offerapprove') {
            this.timeService.offerApproveWorkingArea(modalData).then(result => {
    ...
    getTimeExcel() {
        ...
        this.timeService.getTimeWorkplan(modalData).then(response => {
            ...
            for (let i = 0; i < this.numOfDate; i++) {
                ...
                let calsumAll = Math.ceil((sumT / this.listTime.length) * 100)
                this.timeFTA.push(isNaN(calsumAll)?0:calsumAll)
            }
