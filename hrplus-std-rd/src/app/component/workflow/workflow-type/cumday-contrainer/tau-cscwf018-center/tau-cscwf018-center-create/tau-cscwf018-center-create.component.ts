...
import { TAUCSCWF018CenterDetailComponent } from '../tau-cscwf018-center-detail/tau-cscwf018-center-detail.component'
...
@Component({
  ...
  selector: 'app-tau-cscwf018-center-create',
  templateUrl: './tau-cscwf018-center-create.component.html',
  styleUrls: ['./tau-cscwf018-center-create.component.scss']
})
export class TAUCSCWF018CenterCreateComponent implements OnInit {
  @Input() data: any;
  ...
  setScreenValue() {
    this.workflowService.getDetailByRunNo(this.runno!).subscribe((result) => {
      ...
      this.__wf__dateid = new NgbDate(
        parseInt((this.screenObj["__wf__dateid"] + "").split("-")[2]),
      ...
      this.dynamicComponent = TAUCSCWF018CenterDetailComponent
      this.cdr.markForCheck()
    })
  }
  getEmpCenter() {
    this.workflowService.getEmpCenter().subscribe(result => {
      ...
      if (this.runno && this.screenObj) {
        this.empList.forEach((x: WorkingsModel) => {
          if (x.employeeId == this.screenObj["__wf__employeeid"]) {
