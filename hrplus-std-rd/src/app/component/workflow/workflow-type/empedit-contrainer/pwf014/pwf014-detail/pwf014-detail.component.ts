...
import { ProfileFamilyChildren } from 'src/app/models/employeeprofileall.model';
...
@Component({
  ...
  selector: 'app-pwf014-detail',
  templateUrl: './pwf014-detail.component.html',
  styleUrls: ['./pwf014-detail.component.scss']
})
export class Pwf014DetailComponent implements OnInit {
  page = 0;
  pageSize = 10;
  collectionSize = 0;
  active = 1;
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes) {
      ...
      this.workflowService.getPrefix().then(result => {
        this.prefix = result.map(x => new MyPrefix(x, this.translateService)).sort((a, b) => (a.prefixId! > b.prefixId!) ? 1 : -1);
