...
@Component({
  ...
  selector: 'app-tau-cscwf123-detail',
  templateUrl: './tau-cscwf123-detail.component.html',
  styleUrls: ['./tau-cscwf123-detail.component.scss']
})
export class TAUCSCWF123DetailComponent implements OnInit {
  page = 0;
  pageSize = 10;
  collectionSize = 0;
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes) {
      ...
      this.collectionRecord.forEach((x, i) => {
        ...
        this.shift[i] = this.data.workflowData.screen_value['__wf__tot1$time0$' + (i + 1)];
        this.cause[i] = this.data.workflowData.screen_value['__wf__tot1$ot_cause$' + (i + 1)];
        this.causedesc[i] = this.data.workflowData.screen_value['__wf__tot1$causedesc$' + (i + 1)];
      })
