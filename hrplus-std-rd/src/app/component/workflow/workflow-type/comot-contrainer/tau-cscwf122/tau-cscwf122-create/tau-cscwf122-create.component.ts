...
import { TAUCSCWF122DetailComponent } from '../tau-cscwf122-detail/tau-cscwf122-detail.component'
...
@Component({
  ...
  selector: 'app-tau-cscwf122-create',
  templateUrl: './tau-cscwf122-create.component.html',
  styleUrls: ['./tau-cscwf122-create.component.scss']
})
export class TAUCSCWF122CreateComponent implements OnInit {
  @Input() data: any;
  ...
  onSubmit() {
    ...
    this.__wf__tot1$employeeid$.forEach((x, i) => {
      ...
      screenObj["__wf__tot1$total_time$" + (i + 1)] = this.__wf__tot1$total_time$[i]
      screenObj["__wf__tot1$time0$" + (i + 1)] = this.__wf__tot1$time0$[i]
      screenObj["__wf__tot1$use_in$" + (i + 1)] = this.__wf__tot1$end_date$[i].split("-")[2] + "-" + this.__wf__tot1$end_date$[i].split("-")[1] + "-" + (parseInt(this.__wf__tot1$end_date$[i].split("-")[0]) + 1)
    ...
    this.workflowService.getEmpHr2().then(result => {
      ...
      if (this.runno && this.screenObj) {
        for (let i = 1; this.screenObj["__wf__tot1$employeeid$" + i]; i++) {
          ...
          this.__wf__tot1$start_date$.push(this.screenObj["__wf__tot1$start_date$" + i].split("-")[2] + "-" + this.screenObj["__wf__tot1$start_date$" + i].split("-")[1] + "-" + this.screenObj["__wf__tot1$start_date$" + i].split("-")[0])
          this.__wf__tot1$start_time$.push(this.screenObj["__wf__tot1$start_time$" + i])
          this.__wf__tot1$end_date$.push(this.screenObj["__wf__tot1$end_date$" + i].split("-")[2] + "-" + this.screenObj["__wf__tot1$end_date$" + i].split("-")[1] + "-" + this.screenObj["__wf__tot1$end_date$" + i].split("-")[0])
  ...
  change__wf__tot1$employeeid() {
    if (this.empList!.filter((x: any) => (x.employeeId.toLowerCase() == this.__wf__tot1$employeeid.toLowerCase())).length == 1) {
      this.selectEmpHr(this.empList!.filter((x: any) => (x.employeeId.toLowerCase() == this.__wf__tot1$employeeid.toLowerCase()))[0])
    } else {
