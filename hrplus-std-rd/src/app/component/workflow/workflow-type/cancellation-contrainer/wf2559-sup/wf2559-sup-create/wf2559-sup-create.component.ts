...
// import { Wf2559SupDetailComponent } from '../wf2559-sup-detail/wf2559-sup-detail.component';
...
@Component({
  ...
  selector: 'app-wf2559-sup-create',
  templateUrl: './wf2559-sup-create.component.html',
  styleUrls: ['./wf2559-sup-create.component.scss']
})
export class Wf2559SupCreateComponent implements OnInit {
  @Input() data: any;
  ...
  async requestCancelBossLoop() {
    let i = 0
    while (i < 50 && !this.last) {
      await this.requestCancelBoss(i)
      i++
    }
  }
  async requestCancelBoss(page: number) {
    await this.workflowService.requestCancelBoss(this.token.employeeid, page).then(result => {
      this.last = result.last
      this.cancelBoss = this.cancelBoss.concat(result["content"])
      this.cancelBoss.forEach((x: any, i: number) => {
        let date = new Date()
        date.setTime(this.cancelBoss[i].start_time)
        this.cancelBossDate.push(date)
      });
      this.cancelBoss.sort((a: any, b: any) => (a.doc_no < b.doc_no ? 1 : -1))
      this.cancelBossDate.sort((a: any, b: any) => (a < b ? 1 : -1))
      this.cdr.markForCheck();
    })
  }
  searchCancelBoss() {
    this.cancelBossShow = this.cancelBoss!.filter((x: any) => {
      let date = new Date()
      date.setTime(x.start_time!)
      let formatter = `${("0" + date.getDate()).slice(-2)}-${("0" + (date.getMonth() + 1)).slice(-2)}-${date.getFullYear()}`
      if(x.doc_no?.toLowerCase().includes(this.cancelBossSearch.toLowerCase()) ||
        x.definition.tdesc?.toLowerCase().includes(this.cancelBossSearch.toLowerCase()) ||
        x.definition.edesc?.toLowerCase().includes(this.cancelBossSearch.toLowerCase()) ||
        (x.initiator.fname?.toLowerCase() + ' ' + x.initiator.lname?.toLowerCase()).includes(this.cancelBossSearch.toLowerCase()) ||
        (x.initiator.efname?.toLowerCase() + ' ' + x.initiator.elname?.toLowerCase()).includes(this.cancelBossSearch.toLowerCase()) ||
        formatter.includes(this.cancelBossSearch.toLowerCase())
      ) {
        return x
      }
    });
    this.cancelBossDate = []
    this.cancelBossShow.forEach((x: any, i: number) => {
      let date = new Date()
      date.setTime(this.cancelBossShow[i].start_time)
      this.cancelBossDate.push(date)
    });
    this.pageModal = 1;
    this.collectionSizeModal = this.cancelBossShow.length
  }
  selectCancelBoss(item: any) {
    this.cancelBossSelect = item
    this.__wf__document_number = this.cancelBossSelect.doc_no
    this.__wf__document_name.tdesc = this.cancelBossSelect.definition.tdesc
    this.__wf__document_name.edesc = this.cancelBossSelect.definition.edesc
  ...
  change__wf__document_number() {
    if (this.cancelBoss!.filter((x: any) => (x.doc_no.toLowerCase() == this.__wf__document_number.toLowerCase())).length == 1) {
      this.selectCancelBoss(this.cancelBoss!.filter((x: any) => ((x.doc_no).toLowerCase() == this.__wf__document_number.toLowerCase()))[0])
    } else {
      this.cancelBossSelect = undefined
  ...
  openModal(modal: string, name: string) {
    ...
    if (name == "modalDocuments") {
      this.cancelBossSearch = ""
      this.cancelBossShow = this.cancelBoss
      this.collectionSizeModal = this.cancelBossShow!.length
    }
