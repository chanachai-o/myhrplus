...
@Component({
  ...
  selector: 'app-tau-cscwf007-sup-detail',
  templateUrl: './tau-cscwf007-sup-detail.component.html',
  styleUrls: ['./tau-cscwf007-sup-detail.component.scss']
})
export class TauCscwf007SupDetailComponent implements OnInit {
  @Input() data: any;
  working?: WorkingsModel
  shiftListOld: ShiftListTimeModel[] = []
  shiftSelectOld: ShiftListTimeModel = new MyShiftListTimeModel({}, this.translateService)
  shiftListNew: ShiftListTimeModel[] = []
  shiftSelectNew: ShiftListTimeModel = new MyShiftListTimeModel({}, this.translateService)
  workflowData: any
  inputs = {
    data: {},
  }
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes) {
      ...
      this.wfs.getShiftList().then(result => {
        let shiftOld = result.find(x => x.time0id == this.data.workflowData.screen_value["__wf__old_shift"])
        this.shiftSelectOld = new MyShiftListTimeModel(shiftOld ? shiftOld : {}, this.translateService)
        let shiftNew = result.find(x => x.time0id == this.data.workflowData.screen_value["__wf__new_shift"])
        this.shiftSelectNew = new MyShiftListTimeModel(shiftNew ? shiftNew : {}, this.translateService)
        this.cdr.markForCheck()
      }, error => {
        this.ngbModal.dismissAll()
        this.openAlertModal(this.translateService.instant(error.message) )
      ...
      this.wfs.getShiftListTime(startDate, endDate, this.data.workflowData.screen_value["__wf__old_shift"]).then(result => {
        this.shiftListOld = result.map(x => new MyShiftListTimeModel(x, this.translateService))
        this.cdr.markForCheck()
      }, error => {
        this.ngbModal.dismissAll()
        this.openAlertModal(this.translateService.instant(error.message) )
      })
      this.wfs.getShiftListTime(startDate, endDate, this.data.workflowData.screen_value["__wf__new_shift"]).then(result => {
        this.shiftListNew = result.map(x => new MyShiftListTimeModel(x, this.translateService))
        this.cdr.markForCheck()
      }, error => {
        this.ngbModal.dismissAll()
        this.openAlertModal(this.translateService.instant(error.message) )
