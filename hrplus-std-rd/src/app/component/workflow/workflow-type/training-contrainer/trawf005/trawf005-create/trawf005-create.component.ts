...
@Component({
  selector: 'app-trawf005-create',
  templateUrl: './trawf005-create.component.html',
  styleUrls: ['./trawf005-create.component.scss'],
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
...
export class Trawf005CreateComponent implements OnInit {
  @Input() data: any;
  ...
  setScreenValue() {
    this.workflowService.getDetailByRunNo(this.runno!).subscribe((result) => {
      ...
      this.dynamicComponent = Trawf005Component
  ...
  getTrainIn() {
    if (this.traningId != "") {
      this.workflowService.getTrainInฺById(this.traningId).subscribe(result => {
        // this.trainIn = this.trainIn?.push(re)
        console.log("TRANING", result)
        // this.trainInShow = result;
        this.selectTrainIn(result)
        this.cdr.markForCheck();
      });
    }
  ...
  trainingIdChange(v: string) {
    let trainIn = this.trainIn!.find((x: any) => (x.trainingId.toLowerCase() == v.toLowerCase()));
    if (trainIn) {
  ...
  onSubmit() {
    if (this.selectEmployee) {
      this.employeeID = ''
      this.selectEmployee.forEach(x => {
        this.employeeID = this.employeeID != '' ? this.employeeID + ',' + x.emp.employeeId! : x.emp.employeeId!
      })
    }
    ...
    screenObj["__wf__emp_list"] = this.employeeID;
