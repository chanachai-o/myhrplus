import { ChangeDetectorRef, Component, ElementRef, Injectable, Input, input, OnInit, ViewChild } from '@angular/core';
...
import { TAUCSCWF006DetailComponent } from '../tau-cscwf006-detail/tau-cscwf006-detail.component';
...
@Component({
  ...
  selector: 'app-tau-cscwf006-create',
  templateUrl: './tau-cscwf006-create.component.html',
  styleUrls: ['./tau-cscwf006-create.component.scss']
})
export class TAUCSCWF006CreateComponent implements OnInit {
  ...
  getEmpCenter() {
    this.workflowService.getEmpCenter().subscribe(result => {
      ...
      this.empHrShow = this.empList.sort((a: any, b: any) => (a.employeeId! > b.employeeId!) ? 1 : -1);
  ...
  setScreenValue() {
    this.workflowService.getDetailByRunNo(this.runno!).subscribe((result) => {
      ...
      this.dynamicComponent = TAUCSCWF006DetailComponent
  ...
  getForgetTimeView() {
    this.showView = false;
    let empId = ''
    let stdate = this.selectStartDate
    let endate = this.selectEndDate
    this.empHr.forEach((result: any, index: any) => {
