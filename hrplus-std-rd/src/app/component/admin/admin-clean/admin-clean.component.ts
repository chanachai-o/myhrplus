...
@Component({
  ...
  selector: 'app-admin-clean',
  templateUrl: './admin-clean.component.html',
  styleUrls: ['./admin-clean.component.scss'],
  encapsulation: ViewEncapsulation.None
})
export class AdminCleanComponent implements OnInit {
  ...
  openModal(modalName: string, name: string, other?: any): void {
    ...
    if (name == "alertModal" || name == "confirm archive note") {
  ...
  submit(buttonName: string) {
    if (buttonName == "search") {
      ...
      this.workflowAdminAdminArchiveDocumentPage()
    }
    if (buttonName == 'archive note') {
      if (this.dataList.workflow.list.find(x => x.select === true)) {
        this.openModal(this.alertModal, "confirm archive note", this.translateService.currentLang == "th" ? "คุณต้องการจัดเก็บเอกสาร" : "Do you want to archive note?")
      } else {
        this.openModal(this.alertModal, "alertModal", this.translateService.currentLang == "th" ? "กรุณาเลือกเอกสารที่ต้องการจัดเก็บ" : "Please select note.")
      }
    }
    if (buttonName == 'confirm archive note') {
      this.closeAllModal()
      this.workflowAdminArchive()
    }
  }
  ...
  workflowAdminAdminArchiveDocumentPage() {
    this.subscription[0] = this.workflowService.workflowAdminAdminArchiveDocumentPage(this.dataList.workflow.load.page.toString(), this.dataList.workflow.load.size.toString(), this.dataSelected.textSearch).subscribe(result => {
  ...
  workflowAdminArchive() {
    ...
    this.subscription[2] = this.workflowService.workflowAdminArchive(body).subscribe(result => {
      this.subscription[0].unsubscribe()
