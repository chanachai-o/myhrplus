...
import { NgbDate, NgbDateStruct, NgbDatepickerI18n, NgbDatepickerModule, NgbModal, NgbModalRef, NgbPaginationModule } from "@ng-bootstrap/ng-bootstrap";
...
import { SetWorkoffComponent } from "../../roster/daily-time-attendance-k/modal/set-workoff/set-workoff.component";
import { SetCOffComponent } from "../../roster/daily-time-attendance-k/modal/set-c-off/set-c-off.component";
import { PostLeaveComponent } from "../../roster/daily-time-attendance-k/modal/post-leave/post-leave.component";
import { SetCOTComponent } from "../../roster/daily-time-attendance-k/modal/set-c-ot/set-c-ot.component";
import { SetOTComponent } from "../../roster/daily-time-attendance-k/modal/set-ot/set-ot.component";
import { SetPunchComponent } from "../../roster/daily-time-attendance-k/modal/set-punch/set-punch.component";
import { SetShiftComponent } from "../../roster/daily-time-attendance-k/modal/set-shift/set-shift.component";
...
@Component({
  ...
  imports: [CommonModule, TranslateModule, NgbDatepickerModule, FormsModule, NgbPaginationModule],
    selector: 'app-daily-time-attendance-k-center',
    templateUrl: './daily-time-attendance-k-center.component.html',
    styleUrls: ['./daily-time-attendance-k-center.component.scss']
})
export class DailyTimeAttendanceKCenterComponent implements OnInit {
    page = 1
    pageSize = 10
    ...
    getUserAccessibleWorkarea() {
        ...
        this.userAccessibleWorkareaSubscription = this.workAreaService.getUserAccessibleCenterList().pipe(map(x => x.map(y => new WorkAreaModel(y, this.translateService)))).subscribe(response => {
    ...
    getEmployee() {
        ...
        this.employeeSubscription = this.employeeService.getEmployeeCenter().pipe(map(x => x.map(y => new KerryEmployeeModel(y, this.translateService)))).subscribe(response => {
    ...
    openEmployeeModal() {
        this.employeeModal = this.ngbModal.open(KerryEmployeeModalComponent, { centered: true, backdrop: "static", windowClass: 'dialog-width', size: 'lg' })
        this.employeeModal.componentInstance.employeeList = this.employeeList
        this.employeeModal.componentInstance.employeeListLoading = this.employeeListLoading
    ...
    search() {
        ...
        this.tc1Service.searchCenter(body).pipe(map(x => x.map(y => new KerryTc1EmployeeModel(y, this.translateService)))).subscribe(async response => {
