import { DatePipe, formatDate, FormStyle, getLocaleDayNames, getLocaleMonthNames, registerLocaleData, TranslationWidth } from '@angular/common';
...
import { NgbDate, NgbDateParserFormatter, NgbModule, NgbNavModule } from '@ng-bootstrap/ng-bootstrap';
...
export class EmployeeTimestampComponent implements OnInit {
    page = 0;
    pageSize = 10;
    ...
    loadData() {
        ...
        let datestart = this.parserFormat.format(this.selectStartDate).replace(this.re, '-').split('-');
        let dateend = this.parserFormat.format(this.selectEndDate).replace(this.re, '-').split('-');
        this.timeService.getListPeriod(datestart[2] + "-" + datestart[1] + "-" + datestart[0], dateend[2] + "-" + dateend[1] + "-" + dateend[0]).subscribe(result => {
            this.data = result;
    ...
    getEventGrpId(value: any) {
        if (this.translateService.currentLang == 'th') {
            if (value.eventgrp.tdesc == 'Not Punch') {
                ...
                return value.eventgrp.tdesc
            }
        } else {
            return value.eventgrp.edesc
        }

        // return this.translateService.currentLang == 'th' && value.eventgrp.tdesc == 'Not Punch'
        // ? 'ลืมลงเวลา'
