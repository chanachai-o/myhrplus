...
import { NgbDatepickerModule, NgbPaginationModule, NgbTypeahead, NgbTypeaheadModule } from '@ng-bootstrap/ng-bootstrap';
...
import { EMPLOYEES } from '../sup-timestamp/data-sup-timestamp';
...
@Component({   
  standalone: true,
  imports: [CommonModule, TranslateModule, NgbDatepickerModule, FormsModule, NgbPaginationModule, NgbTypeaheadModule],
  selector: 'app-working-time-detail-history',
  templateUrl: './working-time-detail-history.component.html',
  styleUrls: ['./working-time-detail-history.component.scss']
})

export class WorkingTimeDetailHistoryComponent implements OnInit {
  closeResult = '';
  page = 1;
  pageSize = 50;
  ...
  dataSubordinate:SubordinatesContent[] | undefined;
  map = new Map();
  score = 0;
  scoreOT = 0;
  loading = true;
  ...
  loadData(empId:string) {
    this.dataTime = [];
    this.dataTimeShow = [];
    this.timeService.getListPeriod2(this.start!, this.end!,empId).subscribe(result => {
  ...
  getTotalData() {
    if(this.dataTimeShow){
      this.totalData = this.dataTimeShow!.slice((this.page-1) * this.pageSize, ((this.page-1) * this.pageSize) + this.pageSize);
      this.getAllTotal();
    }
  }
  ...
  refreshData() {
    this.loading = true;
    this.timeService.getSubordinatesHistory(this.start!,this.end!,(this.model?.groupId!)?(this.model?.groupId!):'',this.pageSize,this.page-1).subscribe(result => {
      this.dataSubordinate  = result.content;
      console.log("this.dataSubordinate",result)
      this.loading = false;
      for (let i = 0; i < this.dataSubordinate!.length; i++) {
        this.dataSubordinate![i].timeCurrent!.sort((a, b) => {
      ...
      this.dataSubordinate = this.addDateToTimeCurrent(this.dataSubordinate!);
