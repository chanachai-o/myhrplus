...
@Component({
  ...
  imports: [
    ...
    JobGradeFormComponent, FormsModule, ReactiveFormsModule, EmptyStateComponent
  ],
...
export class JobGradeListComponent implements OnInit {
  public service = inject(JobGradeService);
  ...
  data = signal<any[]>([]);
  showModal = false;
  selectedItem: JobGrade | null = null;
  ...
  loadData() {
    this.service.getAll().subscribe({
      ...
      error: (err) => {
        console.error('[JobGradeList]', err);
  ...
  private updateTranslations() {
    const p = 'company.jobGrade.column.';
    this.headerActions = [
    ...
    this.columns = [
      { field: 'jobGradeId', headerText: this.translate.instant(p + 'id'), width: '140px', isPrimaryKey: true },
  ...
  onDelete(row: any) {
    if (!row?.jobGradeId) { this.notificationService.showError(this.translate.instant(TRANSLATION_KEYS.COMMON.MESSAGES.ERROR.DELETE)); return; }
    const modelFields: (keyof JobGrade)[] = ['jobGradeId', 'companyId', 'tdesc', 'edesc', 'editBy', 'editDate', 'editTime', 'verified'];
    const cleanRow = filterSyncfusionFields<JobGrade>(row, modelFields);
