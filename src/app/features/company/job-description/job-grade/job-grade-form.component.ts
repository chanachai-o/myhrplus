...
export class JobGradeFormComponent implements OnChanges {
  @Input() isOpen = false;
  @Input() data: JobGrade | null = null;
  ...
  constructor() {
    this.form = this.fb.group({
      jobGradeId: ['', [Validators.required, Validators.maxLength(20)]],
  ...
  ngOnChanges() {
    if (this.isOpen) {
      ...
      if (this.data) {
        ...
        this.form.get('jobGradeId')?.disable();
      } else {
        this.form.reset();
        this.form.get('jobGradeId')?.enable();
      }
    }
  }
  ...
  private saveData() {
    ...
    const req$ = this.isEditMode ? this.service.update(formData.jobGradeId, formData) : this.service.create(formData);
    req$.subscribe({
      next: () => {
        setTimeout(() => {
