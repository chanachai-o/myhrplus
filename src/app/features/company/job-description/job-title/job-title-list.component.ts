...
@Component({
  ...
  imports: [
    ...
    JobTitleFormComponent, FormsModule, ReactiveFormsModule, EmptyStateComponent
  ],
...
export class JobTitleListComponent implements OnInit {
  public service = inject(JobTitleService);
  ...
  data = signal<any[]>([]);
  showModal = false;
  selectedItem: JobTitle | null = null;
  ...
  loadData() {
    this.service.getAll().subscribe({
      ...
      error: (err) => {
        console.error('[JobTitleList]', err);
  ...
  private updateTranslations() {
    const p = 'company.jobTitle.column.';
    this.headerActions = [
    ...
    this.columns = [
      { field: 'jobTitleId', headerText: this.translate.instant(p + 'id'), width: '140px', isPrimaryKey: true },
  ...
  onDelete(row: any) {
    if (!row?.jobTitleId) { this.notificationService.showError(this.translate.instant(TRANSLATION_KEYS.COMMON.MESSAGES.ERROR.DELETE)); return; }
    const modelFields: (keyof JobTitle)[] = ['jobTitleId', 'companyId', 'tdesc', 'edesc', 'editBy', 'editDate', 'editTime', 'verified'];
    const cleanRow = filterSyncfusionFields<JobTitle>(row, modelFields);
