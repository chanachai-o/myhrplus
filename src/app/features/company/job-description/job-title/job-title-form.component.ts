...
export class JobTitleFormComponent implements OnChanges {
  @Input() isOpen = false;
  @Input() data: JobTitle | null = null;
  ...
  constructor() {
    this.form = this.fb.group({
      jobTitleId: ['', [Validators.required, Validators.maxLength(20)]],
  ...
  ngOnChanges() {
    if (this.isOpen) {
      ...
      if (this.data) {
        ...
        this.form.get('jobTitleId')?.disable();
      } else {
        this.form.reset();
        this.form.get('jobTitleId')?.enable();
      }
    }
  }
  ...
  private saveData() {
    ...
    const req$ = this.isEditMode ? this.service.update(formData.jobTitleId, formData) : this.service.create(formData);
    req$.subscribe({
      next: () => {
        setTimeout(() => {
