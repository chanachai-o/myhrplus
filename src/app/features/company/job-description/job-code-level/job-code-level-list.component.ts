...
import { JobCodeLevelFormComponent } from './job-code-level-form.component';
...
@Component({
  selector: 'app-job-code-level-list',
  standalone: true,
  imports: [
    ...
    JobCodeLevelFormComponent, FormsModule, ReactiveFormsModule, EmptyStateComponent
  ],
  templateUrl: './job-code-level-list.component.html',
  styles: [`:host { display: flex; flex-direction: column; height: 100%; gap: 1.5rem; }`]
})
export class JobCodeLevelListComponent implements OnInit {
  public service = inject(JobCodeLevelService);
  ...
  data = signal<any[]>([]);
  showModal = false;
  selectedItem: JobCodeLevel | null = null;
  ...
  loadData() {
    this.service.getAll().subscribe({
      ...
      error: (err) => {
        console.error('[JobCodeLevelList]', err);
  ...
  private updateTranslations() {
    const p = 'company.jobCodeLevel.column.';
    this.headerActions = [
    ...
    this.columns = [
      { field: 'jobCodeLevelId', headerText: this.translate.instant(p + 'id'), width: '140px', isPrimaryKey: true },
  ...
  onDelete(row: any) {
    if (!row?.jobCodeLevelId) { this.notificationService.showError(this.translate.instant(TRANSLATION_KEYS.COMMON.MESSAGES.ERROR.DELETE)); return; }
    const modelFields: (keyof JobCodeLevel)[] = ['jobCodeLevelId', 'companyId', 'tdesc', 'edesc', 'editBy', 'editDate', 'editTime', 'verified'];
    const cleanRow = filterSyncfusionFields<JobCodeLevel>(row, modelFields);
