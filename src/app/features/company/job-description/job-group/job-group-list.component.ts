...
import { JobGroupFormComponent } from './job-group-form.component';
...
@Component({
  ...
  imports: [
    ...
    JobGroupFormComponent, FormsModule, ReactiveFormsModule, EmptyStateComponent
  ],
  templateUrl: './job-group-list.component.html',
  styles: [`:host { display: flex; flex-direction: column; height: 100%; gap: 1.5rem; }`]
})
export class JobGroupListComponent implements OnInit {
  public service = inject(JobGroupService);
  ...
  data = signal<any[]>([]);
  showModal = false;
  selectedItem: JobGroup | null = null;
  ...
  loadData() {
    this.service.getAll().subscribe({
      ...
      error: (err) => {
        console.error('[JobGroupList]', err);
  ...
  private updateTranslations() {
    const p = 'company.jobGroup.column.';
    this.headerActions = [
    ...
    this.columns = [
      { field: 'jobGroupId', headerText: this.translate.instant(p + 'id'), width: '140px', isPrimaryKey: true },
  ...
  onDelete(row: any) {
    if (!row?.jobGroupId) { this.notificationService.showError(this.translate.instant(TRANSLATION_KEYS.COMMON.MESSAGES.ERROR.DELETE)); return; }
    const modelFields: (keyof JobGroup)[] = ['jobGroupId', 'companyId', 'tdesc', 'edesc', 'editBy', 'editDate', 'editTime', 'verified'];
    const cleanRow = filterSyncfusionFields<JobGroup>(row, modelFields);
