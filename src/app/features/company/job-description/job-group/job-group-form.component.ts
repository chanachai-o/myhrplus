...
@Component({
  ...
  templateUrl: './job-group-form.component.html'
})
export class JobGroupFormComponent implements OnChanges {
  @Input() isOpen = false;
  @Input() data: JobGroup | null = null;
  @Output() close = new EventEmitter<void>();
  @Output() save = new EventEmitter<void>();

  private fb = inject(FormBuilder);
  private service = inject(JobGroupService);
  ...
  constructor() {
    this.form = this.fb.group({
      jobGroupId: ['', [Validators.required, Validators.maxLength(20)]],
  ...
  ngOnChanges() {
    if (this.isOpen) {
      ...
      if (this.data) {
        ...
        this.form.get('jobGroupId')?.disable();
      } else {
        this.form.reset();
        this.form.get('jobGroupId')?.enable();
      }
    }
  }
  ...
  private saveData() {
    ...
    const req$ = this.isEditMode ? this.service.update(formData.jobGroupId, formData) : this.service.create(formData);
    req$.subscribe({
      next: () => {
        setTimeout(() => {
