...
import { HandbookSetupService } from '../../services/handbook-setup.service';
import { HandbookSetup } from '../../models/handbook-setup.model';
import { HandbookSetupFormComponent } from './handbook-setup-form.component';
...
@Component({
  selector: 'app-handbook-setup-list',
  standalone: true,
  imports: [
    ...
    HandbookSetupFormComponent, FormsModule, ReactiveFormsModule, EmptyStateComponent
  ],
  templateUrl: './handbook-setup-list.component.html',
  styles: [`:host { display: flex; flex-direction: column; height: 100%; gap: 1.5rem; }`]
})
export class HandbookSetupListComponent implements OnInit {
  public service = inject(HandbookSetupService);
  ...
  data = signal<any[]>([]);
  showModal = false;
  selectedItem: HandbookSetup | null = null;
  ...
  loadData() {
    this.service.getAll().subscribe({
      ...
      error: (err) => {
        console.error('[HandbookSetupList]', err);
  ...
  private updateTranslations() {
    const p = 'company.handbookSetup.column.';
    this.headerActions = [
    ...
    this.columns = [
      { field: 'handbookId', headerText: this.translate.instant(p + 'id'), width: '140px', isPrimaryKey: true },
  ...
  onDelete(row: any) {
    if (!row?.handbookId) { this.notificationService.showError(this.translate.instant(TRANSLATION_KEYS.COMMON.MESSAGES.ERROR.DELETE)); return; }
    const modelFields: (keyof HandbookSetup)[] = ['handbookId', 'companyId', 'tdesc', 'edesc', 'editBy', 'editDate', 'editTime', 'verified'];
    const cleanRow = filterSyncfusionFields<HandbookSetup>(row, modelFields);
