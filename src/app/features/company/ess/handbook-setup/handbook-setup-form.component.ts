...
@Component({
  selector: 'app-handbook-setup-form',
  standalone: true,
  ...
  templateUrl: './handbook-setup-form.component.html'
})
export class HandbookSetupFormComponent implements OnChanges {
  @Input() isOpen = false;
  @Input() data: HandbookSetup | null = null;
  @Output() close = new EventEmitter<void>();
  @Output() save = new EventEmitter<void>();

  private fb = inject(FormBuilder);
  private service = inject(HandbookSetupService);
  ...
  constructor() {
    this.form = this.fb.group({
      handbookid: ['', [Validators.required, Validators.maxLength(20)]],
  ...
  ngOnChanges() {
    if (this.isOpen) {
      ...
      if (this.data) {
        ...
        this.form.get('handbookid')?.disable();
      } else {
        this.form.reset();
        this.form.get('handbookid')?.enable();
      }
    }
  }
  ...
  private saveData() {
    ...
    const req$ = this.isEditMode ? this.service.update(formData.handbookid, formData) : this.service.create(formData);
    req$.subscribe({
      next: () => {
        setTimeout(() => {
