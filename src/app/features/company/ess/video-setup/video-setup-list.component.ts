...
import { VideoSetupService } from '../../services/video-setup.service';
import { VideoSetup } from '../../models/video-setup.model';
import { VideoSetupFormComponent } from './video-setup-form.component';
...
@Component({
  ...
  imports: [
    ...
    VideoSetupFormComponent, FormsModule, ReactiveFormsModule, EmptyStateComponent
  ],
  templateUrl: './video-setup-list.component.html',
  styles: [`:host { display: flex; flex-direction: column; height: 100%; gap: 1.5rem; }`]
})
export class VideoSetupListComponent implements OnInit {
  public service = inject(VideoSetupService);
  ...
  data = signal<any[]>([]);
  showModal = false;
  selectedItem: VideoSetup | null = null;
  ...
  loadData() {
    this.service.getAll().subscribe({
      ...
      error: (err) => {
        console.error('[VideoSetupList]', err);
  ...
  private updateTranslations() {
    const p = 'company.videoSetup.column.';
    this.headerActions = [
    ...
    this.columns = [
      { field: 'videoId', headerText: this.translate.instant(p + 'id'), width: '140px', isPrimaryKey: true },
  ...
  onDelete(row: any) {
    if (!row?.videoId) { this.notificationService.showError(this.translate.instant(TRANSLATION_KEYS.COMMON.MESSAGES.ERROR.DELETE)); return; }
    const modelFields: (keyof VideoSetup)[] = ['videoId', 'companyId', 'tdesc', 'edesc', 'editBy', 'editDate', 'editTime', 'verified'];
    const cleanRow = filterSyncfusionFields<VideoSetup>(row, modelFields);
