...
@Component({
  ...
  templateUrl: './video-setup-form.component.html'
})
export class VideoSetupFormComponent implements OnChanges {
  @Input() isOpen = false;
  @Input() data: VideoSetup | null = null;
  @Output() close = new EventEmitter<void>();
  @Output() save = new EventEmitter<void>();

  private fb = inject(FormBuilder);
  private service = inject(VideoSetupService);
  ...
  constructor() {
    this.form = this.fb.group({
      videoid: ['', [Validators.required, Validators.maxLength(20)]],
  ...
  ngOnChanges() {
    if (this.isOpen) {
      ...
      if (this.data) {
        ...
        this.form.get('videoid')?.disable();
      } else {
        this.form.reset();
        this.form.get('videoid')?.enable();
      }
    }
  }
  ...
  private saveData() {
    ...
    const req$ = this.isEditMode ? this.service.update(formData.videoid, formData) : this.service.create(formData);
    req$.subscribe({
      next: () => {
        setTimeout(() => {
