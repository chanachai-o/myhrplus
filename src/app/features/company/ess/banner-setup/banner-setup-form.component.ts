...
@Component({
  selector: 'app-banner-setup-form',
  standalone: true,
  ...
  templateUrl: './banner-setup-form.component.html'
})
export class BannerSetupFormComponent implements OnChanges {
  @Input() isOpen = false;
  @Input() data: BannerSetup | null = null;
  @Output() close = new EventEmitter<void>();
  @Output() save = new EventEmitter<void>();

  private fb = inject(FormBuilder);
  private service = inject(BannerSetupService);
  ...
  constructor() {
    this.form = this.fb.group({
      bannerid: ['', [Validators.required, Validators.maxLength(20)]],
  ...
  ngOnChanges() {
    if (this.isOpen) {
      ...
      if (this.data) {
        ...
        this.form.get('bannerid')?.disable();
      } else {
        this.form.reset();
        this.form.get('bannerid')?.enable();
      }
    }
  }
  ...
  private saveData() {
    ...
    const req$ = this.isEditMode ? this.service.update(formData.bannerid, formData) : this.service.create(formData);
    req$.subscribe({
      next: () => {
        setTimeout(() => {
