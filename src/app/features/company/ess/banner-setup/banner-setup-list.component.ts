...
import { BannerSetupService } from '../../services/banner-setup.service';
import { BannerSetup } from '../../models/banner-setup.model';
import { BannerSetupFormComponent } from './banner-setup-form.component';
...
@Component({
  selector: 'app-banner-setup-list',
  standalone: true,
  imports: [
    ...
    BannerSetupFormComponent, FormsModule, ReactiveFormsModule, EmptyStateComponent
  ],
  templateUrl: './banner-setup-list.component.html',
  styles: [`:host { display: flex; flex-direction: column; height: 100%; gap: 1.5rem; }`]
})
export class BannerSetupListComponent implements OnInit {
  public service = inject(BannerSetupService);
  ...
  data = signal<any[]>([]);
  showModal = false;
  selectedItem: BannerSetup | null = null;
  ...
  loadData() {
    this.service.getAll().subscribe({
      ...
      error: (err) => {
        console.error('[BannerSetupList]', err);
  ...
  private updateTranslations() {
    const p = 'company.bannerSetup.column.';
    this.headerActions = [
    ...
    this.columns = [
      { field: 'bannerId', headerText: this.translate.instant(p + 'id'), width: '140px', isPrimaryKey: true },
  ...
  onDelete(row: any) {
    if (!row?.bannerId) { this.notificationService.showError(this.translate.instant(TRANSLATION_KEYS.COMMON.MESSAGES.ERROR.DELETE)); return; }
    const modelFields: (keyof BannerSetup)[] = ['bannerId', 'companyId', 'tdesc', 'edesc', 'editBy', 'editDate', 'editTime', 'verified'];
    const cleanRow = filterSyncfusionFields<BannerSetup>(row, modelFields);
