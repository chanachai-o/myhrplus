...
import { EventSetupService } from '../../services/event-setup.service';
import { EventSetup } from '../../models/event-setup.model';
import { EventSetupFormComponent } from './event-setup-form.component';
...
@Component({
  ...
  imports: [
    ...
    EventSetupFormComponent, FormsModule, ReactiveFormsModule, EmptyStateComponent
  ],
  templateUrl: './event-setup-list.component.html',
  styles: [`:host { display: flex; flex-direction: column; height: 100%; gap: 1.5rem; }`]
})
export class EventSetupListComponent implements OnInit {
  public service = inject(EventSetupService);
  ...
  data = signal<any[]>([]);
  showModal = false;
  selectedItem: EventSetup | null = null;
  ...
  loadData() {
    this.service.getAll().subscribe({
      ...
      error: (err) => {
        console.error('[EventSetupList]', err);
  ...
  private updateTranslations() {
    const p = 'company.eventSetup.column.';
    this.headerActions = [
    ...
    this.columns = [
      { field: 'eventId', headerText: this.translate.instant(p + 'id'), width: '140px', isPrimaryKey: true },
  ...
  onDelete(row: any) {
    if (!row?.eventId) { this.notificationService.showError(this.translate.instant(TRANSLATION_KEYS.COMMON.MESSAGES.ERROR.DELETE)); return; }
    const modelFields: (keyof EventSetup)[] = ['eventId', 'companyId', 'tdesc', 'edesc', 'editBy', 'editDate', 'editTime', 'verified'];
    const cleanRow = filterSyncfusionFields<EventSetup>(row, modelFields);
