...
@Component({
  ...
  templateUrl: './event-setup-form.component.html'
})
export class EventSetupFormComponent implements OnChanges {
  @Input() isOpen = false;
  @Input() data: EventSetup | null = null;
  @Output() close = new EventEmitter<void>();
  @Output() save = new EventEmitter<void>();

  private fb = inject(FormBuilder);
  private service = inject(EventSetupService);
  ...
  constructor() {
    this.form = this.fb.group({
      eventid: ['', [Validators.required, Validators.maxLength(20)]],
  ...
  private saveData() {
    ...
    const req$ = this.isEditMode ? this.service.update(formData.eventid, formData) : this.service.create(formData);
    req$.subscribe({
      next: () => {
        setTimeout(() => {
