...
export class WorkareaStoreListComponent implements OnInit {
  ...
  loadData() {
    this.service.getAll().subscribe({
      ...
      error: (err) => {
        console.error('[WorkareaStoreList] Error loading data:', err);
  ...
  private updateTranslations() {
    const prefix = 'company.workareaStore.column.';
    this.headerActions = [
      {
    ...
    this.columns = [
      { field: 'workareaid', headerText: this.translate.instant(prefix + 'workareaId'), width: '150px', isPrimaryKey: true },
      ...
      { field: 'groupid', headerText: this.translate.instant(prefix + 'groupId'), width: '120px' },
      { field: 'zonetypeid', headerText: this.translate.instant(prefix + 'zoneTypeId'), width: '120px' },
  ...
  onDelete(row: any) {
    ...
    const modelFields: (keyof WorkareaStore)[] = [
      'workareaid', 'companyid', 'tdesc', 'edesc', 'groupid', 'zonetypeid', 'work_status', 'edit_by', 'edit_date', 'edit_time', 'verified'
    ];
    const cleanRow = filterSyncfusionFields<WorkareaStore>(row, modelFields);
