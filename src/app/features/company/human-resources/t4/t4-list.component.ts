...
export class T4ListComponent implements OnInit {
  ...
  loadData() {
    this.service.getAll().subscribe({
      ...
      error: (err) => {
        console.error('[T4List] Error loading data:', err);
  ...
  private updateTranslations() {
    const prefix = 'company.t4.column.';
    this.headerActions = [
      {
    ...
    this.columns = [
      { field: 'bu7id', headerText: this.translate.instant(prefix + 'bu7Id'), width: '120px', isPrimaryKey: true },
  ...
  onDelete(row: any) {
    ...
    const modelFields: (keyof T4)[] = [
      'bu7id', 'companyid', 'parent', 'tdesc', 'edesc', 'short_name', 'active', 'build_date', 'expire_date',
      'edit_by', 'edit_date', 'edit_time', 'verified'
    ];
    const cleanRow = filterSyncfusionFields<T4>(row, modelFields);
