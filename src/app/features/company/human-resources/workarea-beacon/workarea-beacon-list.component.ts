...
export class WorkareaBeaconListComponent implements OnInit {
  ...
  loadData() {
    this.service.getAll().subscribe({
      ...
      error: (err) => {
        console.error('[WorkareaBeaconList] Error loading data:', err);
  ...
  private updateTranslations() {
    const prefix = 'company.workareaBeacon.column.';
    this.headerActions = [
      {
    ...
    this.columns = [
      ...
      { field: 'beacon_uuid', headerText: this.translate.instant(prefix + 'beaconUuid'), width: '200px' },
      { field: 'beacon_address', headerText: this.translate.instant(prefix + 'beaconAddress'), width: '150px' },
      { field: 'beacon_name', headerText: this.translate.instant(prefix + 'beaconName'), width: '150px' },
  ...
  onDelete(row: any) {
    ...
    const modelFields: (keyof WorkareaBeacon)[] = [
      'workareaid', 'companyid', 'line_no', 'beacon_uuid', 'beacon_address', 'beacon_name', 'latitude', 'longitude', 'edit_by', 'edit_date', 'edit_time', 'verified'
    ];
    const cleanRow = filterSyncfusionFields<WorkareaBeacon>(row, modelFields);
