...
export class PositionListComponent implements OnInit {
  public service = inject(PositionService);
  ...
  showModal = false;
  selectedItem: PositionModel | null = null;
  ...
  loadData(page: number = this.currentPage, size: number = this.pageSize) {
    this.service.getAllWithPagination({ page, size }).subscribe({
      ...
      error: (err) => {
        console.error('[PositionList] Error loading data:', err);
  ...
  onActionBegin(event: any) {
    if (event.requestType === 'paging') {
      const newPage = (event.currentPage as number) - 1;
  ...
  private updateTranslations() {
    ...
    this.columns = [
      { field: 'positionId', headerText: this.translate.instant('features.company.entities.position.column.positionId'), width: '150px', isPrimaryKey: true },
      { field: 'tdesc', headerText: this.translate.instant('features.company.entities.position.column.tdesc'), width: '250px', minWidth: 200 },
      { field: 'edesc', headerText: this.translate.instant('features.company.entities.position.column.edesc'), width: '250px', minWidth: 200 },
      { field: 'editDate', headerText: this.translate.instant('features.company.entities.position.column.editDate'), type: 'date' as const, width: '180px', format: 'dd/MM/yyyy' }
    ];
  ...
  onDelete(row: any) {
    ...
    const modelFields: (keyof PositionModel)[] = [
      'positionId', 'tdesc', 'edesc', 'active', 'editBy', 'editDate', 'editTime', 'verified', 'companyId'
    ];
    const cleanRow = filterSyncfusionFields<PositionModel>(row, modelFields);
