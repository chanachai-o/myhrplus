...
export class CompanyModelFormComponent implements OnChanges {
  @Input() isOpen = false;
  @Input() data: CompanyModel | null = null;
  ...
  constructor() {
    this.form = this.fb.group({
      ...
      headCompanyModel: ['', Validators.maxLength(10)],
      branchTax: ['', Validators.maxLength(10)],
      // Required fields
      isCompany: ['1', Validators.required],
      isBranch: ['']
    });
  }
  ...
  ngOnChanges() {
    if (this.isOpen) {
      ...
      if (this.data) {
        const formData = {
          ...this.data,
          isCompany: this.data.isCompany || '1',
          isBranch: this.data.isBranch || ''
        };
        this.form.patchValue(formData);
        this.form.get('branchId')?.disable(); // PK cannot be changed
        this.form.get('companyId')?.disable(); // CompanyModel ID is readonly
      } else {
        this.form.reset({
          isCompany: '1',
          isBranch: ''
