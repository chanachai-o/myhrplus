...
export class SectionFormComponent implements OnChanges {
  ...
  @Input() isOpen = false;
  @Input() data: SectionModel | null = null;
  ...
  constructor() {
    this.form = this.fb.group({
      bu3id: ['', [Validators.required, Validators.maxLength(10)]],
      companyid: ['', Validators.required],
      parent: ['', [Validators.required, Validators.maxLength(10)]], // Required parent (Department)
      ...
      sort_number: [0],
      bu3sup: ['', Validators.maxLength(10)],
  ...
  ngOnChanges() {
    if (this.isOpen) {
      ...
      if (this.data) {
        ...
        this.form.get('bu3id')?.disable(); // PK cannot be changed
  ...
  private saveData() {
    ...
    const request$ = this.isEditMode
      ? this.service.update(formData.bu3id, formData)
