...
export class AssetModelListComponent implements OnInit {
  ...
  data = signal<any[]>([]);
  showModal = false;
  selectedItem: AssetModel | null = null;
  ...
  loadData() {
    this.service.getAll().subscribe({
      ...
      error: (err: unknown) => {
        console.error('[AssetModelList] Error loading data:', err);
  ...
  private updateTranslations() {
    ...
    this.columns = [
      { field: 'assetId', headerText: this.translate.instant('company.companyAssetModel.column.assetId'), width: '100px', isPrimaryKey: true },
      { field: 'tdesc', headerText: this.translate.instant('company.companyAssetModel.column.tdesc'), width: '200px' },
      { field: 'astype', headerText: this.translate.instant('company.companyAssetModel.column.astype'), width: '100px' },
      { field: 'status', headerText: this.translate.instant('company.companyAssetModel.column.status'), width: '80px' },
      { field: 'owner', headerText: this.translate.instant('company.companyAssetModel.column.owner'), width: '120px' }
    ];
  ...
  onDelete(row: any) {
    ...
    const modelFields: (keyof AssetModel)[] = [
      'companyId', 'assetId', 'tdesc', 'edesc', 'astype', 'fine', 'remarks', 'owner', 'status', 'reservation'
    ];
    const cleanRow = filterSyncfusionFields<AssetModel>(row, modelFields);
