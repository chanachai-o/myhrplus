...
export class BrandStoreListComponent implements OnInit {
  ...
  loadData() {
    this.service.getAll().subscribe({
      ...
      error: (err) => {
        console.error('[BrandStoreList] Error loading data:', err);
  ...
  private updateTranslations() {
    const prefix = 'company.brandStore.column.';
    this.headerActions = [
      {
    ...
    this.columns = [
      { field: 'brandstoreid', headerText: this.translate.instant(prefix + 'brandStoreId'), width: '150px', isPrimaryKey: true },
  ...
  onDelete(row: any) {
    ...
    const modelFields: (keyof BrandStore)[] = [
      'brandstoreid', 'companyid', 'tdesc', 'edesc', 'edit_by', 'edit_date', 'edit_time', 'verified'
    ];
    const cleanRow = filterSyncfusionFields<BrandStore>(row, modelFields);
