...
export class ZoneTypeListComponent implements OnInit {
  ...
  loadData() {
    this.service.getAll().subscribe({
      ...
      error: (err) => {
        console.error('[ZoneTypeList] Error loading data:', err);
  ...
  private updateTranslations() {
    const prefix = 'company.zoneType.column.';
    this.headerActions = [
      {
    ...
    this.columns = [
      { field: 'zonetypeid', headerText: this.translate.instant(prefix + 'zoneTypeId'), width: '150px', isPrimaryKey: true },
      ...
      { field: 'brand_support', headerText: this.translate.instant(prefix + 'brandSupport'), width: '150px' },
  ...
  onDelete(row: any) {
    ...
    const modelFields: (keyof ZoneType)[] = [
      'zonetypeid', 'companyid', 'tdesc', 'edesc', 'brand_support', 'edit_by', 'edit_date', 'edit_time', 'verified'
    ];
    const cleanRow = filterSyncfusionFields<ZoneType>(row, modelFields);
