...
export class BranchSocialSecurityListComponent implements OnInit {
  ...
  loadData() {
    this.service.getAll().subscribe({
      ...
      error: (err) => {
        console.error('[BranchSocialSecurityList] Error loading data:', err);
  ...
  private updateTranslations() {
    ...
    this.columns = [
      { field: 'branch_no', headerText: this.translate.instant('features.company.entities.branchSocialSecurity.column.branchNo'), width: '150px', isPrimaryKey: true },
      ...
      { field: 'edit_date', headerText: this.translate.instant('features.company.entities.branchSocialSecurity.column.editDate'), type: 'date' as const, width: '120px', format: 'dd/MM/yyyy' }
    ];
  ...
  onDelete(row: any) {
    ...
    const modelFields: (keyof BranchSocialSecurity)[] = [
      'branch_no', 'companyid', 'tdesc', 'edesc', 'social_code', 'social_sub_code', 'tel', 'edit_date'
    ];
    const cleanRow = filterSyncfusionFields<BranchSocialSecurity>(row, modelFields);
