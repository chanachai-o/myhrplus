...
export class PLListComponent implements OnInit {
  ...
  loadData() {
    this.service.getAll().subscribe({
      ...
      error: (err) => {
        console.error('[PLList] Error loading data:', err);
  ...
  private updateTranslations() {
    const prefix = 'company.pl.column.';
    this.headerActions = [
      {
    ...
    this.columns = [
      { field: 'plid', headerText: this.translate.instant(prefix + 'plId'), width: '120px', isPrimaryKey: true },
      ...
      { field: 'jobgradeid', headerText: this.translate.instant(prefix + 'jobGradeId'), width: '120px' },
      { field: 'band', headerText: this.translate.instant(prefix + 'band'), width: '100px' },
      { field: 'order_no', headerText: this.translate.instant(prefix + 'orderNo'), width: '100px' },
  ...
  onDelete(row: any) {
    ...
    const modelFields: (keyof PL)[] = [
      'plid', 'companyid', 'tdesc', 'edesc', 'jobgradeid', 'band', 'order_no', 'edit_by', 'edit_date', 'edit_time', 'verified'
    ];
    const cleanRow = filterSyncfusionFields<PL>(row, modelFields);
