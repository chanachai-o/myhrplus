...
export class WorkingAreaListComponent implements OnInit {
  ...
  loadData() {
    this.service.getAll().subscribe({
      ...
      error: (err) => {
        console.error('[WorkingAreaList] Error loading data:', err);
  ...
  private updateTranslations() {
    const prefix = 'company.workingArea.column.';
    this.headerActions = [
      {
    ...
    this.columns = [
      ...
      { field: 'DATEST', headerText: this.translate.instant(prefix + 'dateStart'), type: 'date' as const, width: '120px', format: 'dd/MM/yyyy' },
      { field: 'DATEEN', headerText: this.translate.instant(prefix + 'dateEnd'), type: 'date' as const, width: '120px', format: 'dd/MM/yyyy' },
      { field: 'BRAND_SUPPORT', headerText: this.translate.instant(prefix + 'brandSupport'), width: '120px' },
  ...
  onDelete(row: any) {
    ...
    const modelFields: (keyof WorkingArea)[] = [
      'workareaid', 'companyid', 'tdesc', 'edesc', 'DATEST', 'DATEEN', 'BRAND_SUPPORT', 'edit_by', 'edit_date', 'edit_time', 'verified'
    ];
    const cleanRow = filterSyncfusionFields<WorkingArea>(row, modelFields);
