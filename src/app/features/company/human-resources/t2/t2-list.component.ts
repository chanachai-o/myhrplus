...
export class T2ListComponent implements OnInit {
  ...
  loadData() {
    this.service.getAll().subscribe({
      ...
      error: (err) => {
        console.error('[T2List] Error loading data:', err);
  ...
  private updateTranslations() {
    const prefix = 'company.t2.column.';
    this.headerActions = [
      {
    ...
    this.columns = [
      { field: 'bu5id', headerText: this.translate.instant(prefix + 'bu5Id'), width: '120px', isPrimaryKey: true },
  ...
  onDelete(row: any) {
    ...
    const modelFields: (keyof T2)[] = [
      'bu5id', 'companyid', 'parent', 'tdesc', 'edesc', 'short_name', 'active', 'build_date', 'expire_date',
      'edit_by', 'edit_date', 'edit_time', 'verified'
    ];
    const cleanRow = filterSyncfusionFields<T2>(row, modelFields);
