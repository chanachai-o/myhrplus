...
export class T2FormComponent implements OnChanges {
  ...
  private service = inject(T2Service);
  ...
  constructor() {
    this.form = this.fb.group({
      bu5id: ['', [Validators.required, Validators.maxLength(10)]],
      companyid: ['', Validators.required],
      parent: ['', [Validators.required, Validators.maxLength(10)]], // Required parent (Team)
      ...
      sort_number: [0],
      bu5sup: ['', Validators.maxLength(10)],
  ...
  ngOnChanges() {
    if (this.isOpen) {
      ...
      if (this.data) {
        ...
        this.form.get('bu5id')?.disable(); // PK cannot be changed
  ...
  private saveData() {
    ...
    const request$ = this.isEditMode
      ? this.service.update(formData.bu5id, formData)
