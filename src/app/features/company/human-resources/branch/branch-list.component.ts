...
import { BranchModelFormComponent } from './branch-form.component';
...
export class BranchModelListComponent implements OnInit {
  ...
  data = signal<any[]>([]);
  showModal = false;
  selectedItem: BranchModel | null = null;
  ...
  loadData() {
    this.service.getAll().subscribe({
      ...
      error: (err: unknown) => {
        console.error('[BranchModelList] Error loading data:', err);
  ...
  private updateTranslations() {
    const prefix = 'features.company.entities.branch.column.';
    this.headerActions = [
      {
    ...
    this.columns = [
      ...
      { field: 'headcompany', headerText: this.translate.instant(prefix + 'headCompany'), width: '120px' },
      { field: 'com_type', headerText: this.translate.instant(prefix + 'comType'), width: '100px' },
  ...
  onDelete(row: any) {
    ...
    const modelFields: (keyof BranchModel)[] = [
      'branchId', 'companyId', 'headCompany', 'tdesc', 'edesc', 'comType', 'tel', 'editBy', 'editDate', 'editTime', 'verified'
    ];
    const cleanRow = filterSyncfusionFields<BranchModel>(row, modelFields);
