...
export class TeamFormComponent implements OnChanges {
  ...
  constructor() {
    this.form = this.fb.group({
      bu4id: ['', [Validators.required, Validators.maxLength(10)]],
      companyid: ['', Validators.required],
      parent: ['', [Validators.required, Validators.maxLength(10)]], // Required parent (Section)
      ...
      sort_number: [0],
      bu4sup: ['', Validators.maxLength(10)],
  ...
  ngOnChanges() {
    if (this.isOpen) {
      ...
      if (this.data) {
        ...
        this.form.get('bu4id')?.disable(); // PK cannot be changed
  ...
  private saveData() {
    ...
    const request$ = this.isEditMode
      ? this.service.update(formData.bu4id, formData)
