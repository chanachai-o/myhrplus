...
export class ApproveLevelListComponent implements OnInit {
  ...
  loadData() {
    this.service.getAll().subscribe({
      ...
      error: (err) => {
        console.error('[ApproveLevelList] Error loading data:', err);
  ...
  private updateTranslations() {
    const prefix = 'company.approveLevel.column.';
    this.headerActions = [
      {
    ...
    this.columns = [
      { field: 'approveLevelId', headerText: this.translate.instant(prefix + 'approveLevelId'), width: '150px', isPrimaryKey: true },
  ...
  onDelete(row: any) {
    ...
    if (!row.approveLevelId) {
      this.notificationService.showError(this.translate.instant(TRANSLATION_KEYS.COMMON.MESSAGES.ERROR.DELETE));
      return;
    }

    const modelFields: (keyof ApproveLevel)[] = [
      'approveLevelId', 'companyId', 'tdesc', 'edesc', 'editBy', 'editDate', 'editTime', 'verified'
    ];
    const cleanRow = filterSyncfusionFields<ApproveLevel>(row, modelFields);
