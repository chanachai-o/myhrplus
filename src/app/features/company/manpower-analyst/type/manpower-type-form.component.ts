...
@Component({
  selector: 'app-manpower-type-form',
  standalone: true,
  ...
  templateUrl: './manpower-type-form.component.html'
})
export class ManpowerTypeFormComponent implements OnChanges {
  @Input() isOpen = false;
  @Input() data: ManpowerType | null = null;
  @Output() close = new EventEmitter<void>();
  @Output() save = new EventEmitter<void>();

  private fb = inject(FormBuilder);
  private service = inject(ManpowerTypeService);
  ...
  constructor() {
    this.form = this.fb.group({
      manpowerTypeId: ['', [Validators.required, Validators.maxLength(20)]],
  ...
  ngOnChanges() {
    if (this.isOpen) {
      ...
      if (this.data) {
        ...
        this.form.get('manpowertypeid')?.disable();
      } else {
        this.form.reset();
        this.form.get('manpowerTypeId')?.enable();
      }
    }
  }
  ...
  private saveData() {
    ...
    const req$ = this.isEditMode ? this.service.update(formData.manpowerTypeId, formData) : this.service.create(formData);
    req$.subscribe({
      next: () => {
        setTimeout(() => {
