...
import { ManpowerTypeService } from '../../services/manpower-type.service';
import { ManpowerType } from '../../models/manpower-type.model';
import { ManpowerTypeFormComponent } from './manpower-type-form.component';
...
@Component({
  selector: 'app-manpower-type-list',
  standalone: true,
  imports: [
    ...
    ManpowerTypeFormComponent, FormsModule, ReactiveFormsModule, EmptyStateComponent
  ],
  templateUrl: './manpower-type-list.component.html',
  styles: [`:host { display: flex; flex-direction: column; height: 100%; gap: 1.5rem; }`]
})
export class ManpowerTypeListComponent implements OnInit {
  public service = inject(ManpowerTypeService);
  ...
  data = signal<any[]>([]);
  showModal = false;
  selectedItem: ManpowerType | null = null;
  ...
  loadData() {
    this.service.getAll().subscribe({
      ...
      error: (err) => {
        console.error('[ManpowerTypeList]', err);
  ...
  private updateTranslations() {
    const p = 'company.manpowerType.column.';
    this.headerActions = [
    ...
    this.columns = [
      { field: 'manpowerTypeId', headerText: this.translate.instant(p + 'id'), width: '140px', isPrimaryKey: true },
  ...
  onDelete(row: any) {
    if (!row?.manpowerTypeId) { this.notificationService.showError(this.translate.instant(TRANSLATION_KEYS.COMMON.MESSAGES.ERROR.DELETE)); return; }
    const modelFields: (keyof ManpowerType)[] = ['manpowerTypeId', 'companyId', 'tdesc', 'edesc', 'editBy', 'editDate', 'editTime', 'verified'];
    const cleanRow = filterSyncfusionFields<ManpowerType>(row, modelFields);
