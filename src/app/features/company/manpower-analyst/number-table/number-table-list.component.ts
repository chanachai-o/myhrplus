...
import { ManpowerNumberTableService } from '../../services/manpower-number-table.service';
import { ManpowerNumberTable } from '../../models/manpower-number-table.model';
import { NumberTableFormComponent } from './number-table-form.component';
...
@Component({
  selector: 'app-number-table-list',
  standalone: true,
  imports: [
    ...
    NumberTableFormComponent, FormsModule, ReactiveFormsModule, EmptyStateComponent
  ],
  templateUrl: './number-table-list.component.html',
  styles: [`:host { display: flex; flex-direction: column; height: 100%; gap: 1.5rem; }`]
})
export class NumberTableListComponent implements OnInit {
  public service = inject(ManpowerNumberTableService);
  ...
  data = signal<any[]>([]);
  showModal = false;
  selectedItem: ManpowerNumberTable | null = null;
  ...
  loadData() {
    this.service.getAll().subscribe({
      ...
      error: (err) => {
        console.error('[NumberTableList]', err);
  ...
  private updateTranslations() {
    const p = 'company.manpowerNumberTable.column.';
    this.headerActions = [
    ...
    this.columns = [
      { field: 'manpowerNumberTableId', headerText: this.translate.instant(p + 'id'), width: '140px', isPrimaryKey: true },
  ...
  onDelete(row: any) {
    if (!row?.manpowerNumberTableId) { this.notificationService.showError(this.translate.instant(TRANSLATION_KEYS.COMMON.MESSAGES.ERROR.DELETE)); return; }
    const modelFields: (keyof ManpowerNumberTable)[] = ['manpowerNumberTableId', 'companyId', 'tdesc', 'edesc', 'editBy', 'editDate', 'editTime', 'verified'];
    const cleanRow = filterSyncfusionFields<ManpowerNumberTable>(row, modelFields);
