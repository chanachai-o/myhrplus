...
@Component({
  selector: 'app-number-table-form',
  standalone: true,
  ...
  templateUrl: './number-table-form.component.html'
})
export class NumberTableFormComponent implements OnChanges {
  @Input() isOpen = false;
  @Input() data: ManpowerNumberTable | null = null;
  @Output() close = new EventEmitter<void>();
  @Output() save = new EventEmitter<void>();

  private fb = inject(FormBuilder);
  private service = inject(ManpowerNumberTableService);
  ...
  constructor() {
    this.form = this.fb.group({
      manpowerNumberTableId: ['', [Validators.required, Validators.maxLength(20)]],
  ...
  ngOnChanges() {
    if (this.isOpen) {
      ...
      if (this.data) {
        ...
        this.form.get('manpowerNumberTableId')?.disable();
      } else {
        this.form.reset();
        this.form.get('manpowerNumberTableId')?.enable();
      }
    }
  }
  ...
  private saveData() {
    ...
    const req$ = this.isEditMode ? this.service.update(formData.manpowerNumberTableId, formData) : this.service.create(formData);
    req$.subscribe({
      next: () => {
        setTimeout(() => {
