...
import { ManpowerNumberDataService } from '../../services/manpower-number-data.service';
import { ManpowerNumberData } from '../../models/manpower-number-data.model';
import { NumberDataFormComponent } from './number-data-form.component';
...
@Component({
  ...
  imports: [
    ...
    NumberDataFormComponent, FormsModule, ReactiveFormsModule, EmptyStateComponent
  ],
  templateUrl: './number-data-list.component.html',
  styles: [`:host { display: flex; flex-direction: column; height: 100%; gap: 1.5rem; }`]
})
export class NumberDataListComponent implements OnInit {
  public service = inject(ManpowerNumberDataService);
  ...
  data = signal<any[]>([]);
  showModal = false;
  selectedItem: ManpowerNumberData | null = null;
  ...
  loadData() {
    this.service.getAll().subscribe({
      ...
      error: (err) => {
        console.error('[NumberDataList]', err);
  ...
  private updateTranslations() {
    const p = 'company.manpowerNumberData.column.';
    this.headerActions = [
    ...
    this.columns = [
      { field: 'manpowerNumberDataId', headerText: this.translate.instant(p + 'id'), width: '140px', isPrimaryKey: true },
  ...
  onDelete(row: any) {
    if (!row?.manpowerNumberDataId) { this.notificationService.showError(this.translate.instant(TRANSLATION_KEYS.COMMON.MESSAGES.ERROR.DELETE)); return; }
    const modelFields: (keyof ManpowerNumberData)[] = ['manpowerNumberDataId', 'companyId', 'tdesc', 'edesc', 'editBy', 'editDate', 'editTime', 'verified'];
    const cleanRow = filterSyncfusionFields<ManpowerNumberData>(row, modelFields);
