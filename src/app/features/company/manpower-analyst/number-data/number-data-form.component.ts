...
@Component({
  ...
  templateUrl: './number-data-form.component.html'
})
export class NumberDataFormComponent implements OnChanges {
  @Input() isOpen = false;
  @Input() data: ManpowerNumberData | null = null;
  @Output() close = new EventEmitter<void>();
  @Output() save = new EventEmitter<void>();

  private fb = inject(FormBuilder);
  private service = inject(ManpowerNumberDataService);
  ...
  constructor() {
    this.form = this.fb.group({
      manpowerNumberDataId: ['', [Validators.required, Validators.maxLength(20)]],
  ...
  ngOnChanges() {
    if (this.isOpen) {
      ...
      if (this.data) {
        ...
        this.form.get('manpowerNumberDataId')?.disable();
      } else {
        this.form.reset();
        this.form.get('manpowerNumberDataId')?.enable();
      }
    }
  }
  ...
  private saveData() {
    ...
    const req$ = this.isEditMode ? this.service.update(formData.manpowerNumberDataId, formData) : this.service.create(formData);
    req$.subscribe({
      next: () => {
        setTimeout(() => {
