...
import { ManpowerNumberDetailService } from '../../services/manpower-number-detail.service';
import { ManpowerNumberDetail } from '../../models/manpower-number-detail.model';
import { NumberDetailFormComponent } from './number-detail-form.component';
...
@Component({
  selector: 'app-number-detail-list',
  standalone: true,
  imports: [
    ...
    NumberDetailFormComponent, FormsModule, ReactiveFormsModule, EmptyStateComponent
  ],
  templateUrl: './number-detail-list.component.html',
  styles: [`:host { display: flex; flex-direction: column; height: 100%; gap: 1.5rem; }`]
})
export class NumberDetailListComponent implements OnInit {
  public service = inject(ManpowerNumberDetailService);
  ...
  data = signal<any[]>([]);
  showModal = false;
  selectedItem: ManpowerNumberDetail | null = null;
  ...
  loadData() {
    this.service.getAll().subscribe({
      ...
      error: (err) => {
        console.error('[NumberDetailList]', err);
  ...
  private updateTranslations() {
    const p = 'company.manpowerNumberDetail.column.';
    this.headerActions = [
    ...
    this.columns = [
      { field: 'manpowerNumberDetailId', headerText: this.translate.instant(p + 'id'), width: '140px', isPrimaryKey: true },
  ...
  onDelete(row: any) {
    if (!row?.manpowerNumberDetailId) { this.notificationService.showError(this.translate.instant(TRANSLATION_KEYS.COMMON.MESSAGES.ERROR.DELETE)); return; }
    const modelFields: (keyof ManpowerNumberDetail)[] = ['manpowerNumberDetailId', 'companyId', 'tdesc', 'edesc', 'editBy', 'editDate', 'editTime', 'verified'];
    const cleanRow = filterSyncfusionFields<ManpowerNumberDetail>(row, modelFields);
