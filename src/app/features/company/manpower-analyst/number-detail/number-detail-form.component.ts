...
@Component({
  selector: 'app-number-detail-form',
  standalone: true,
  ...
  templateUrl: './number-detail-form.component.html'
})
export class NumberDetailFormComponent implements OnChanges {
  @Input() isOpen = false;
  @Input() data: ManpowerNumberDetail | null = null;
  @Output() close = new EventEmitter<void>();
  @Output() save = new EventEmitter<void>();

  private fb = inject(FormBuilder);
  private service = inject(ManpowerNumberDetailService);
  ...
  constructor() {
    this.form = this.fb.group({
      manpowerNumberDetailId: ['', [Validators.required, Validators.maxLength(20)]],
  ...
  ngOnChanges() {
    if (this.isOpen) {
      ...
      if (this.data) {
        ...
        this.form.get('numberdetailid')?.disable();
      } else {
        this.form.reset();
        this.form.get('manpowerNumberDetailId')?.enable();
      }
    }
  }
  ...
  private saveData() {
    ...
    const req$ = this.isEditMode ? this.service.update(formData.manpowerNumberDetailId, formData) : this.service.create(formData);
    req$.subscribe({
      next: () => {
        setTimeout(() => {
