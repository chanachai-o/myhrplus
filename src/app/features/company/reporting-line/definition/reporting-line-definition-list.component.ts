...
import { ReportingLineDefinitionService } from '../../services/reporting-line-definition.service';
import { ReportingLineDefinition } from '../../models/reporting-line-definition.model';
import { ReportingLineDefinitionFormComponent } from './reporting-line-definition-form.component';
...
@Component({
  selector: 'app-reporting-line-definition-list',
  standalone: true,
  imports: [
    ...
    ReportingLineDefinitionFormComponent, FormsModule, ReactiveFormsModule, EmptyStateComponent
  ],
  templateUrl: './reporting-line-definition-list.component.html',
  styles: [`:host { display: flex; flex-direction: column; height: 100%; gap: 1.5rem; }`]
})
export class ReportingLineDefinitionListComponent implements OnInit {
  public service = inject(ReportingLineDefinitionService);
  ...
  data = signal<any[]>([]);
  showModal = false;
  selectedItem: ReportingLineDefinition | null = null;
  ...
  loadData() {
    this.service.getAll().subscribe({
      ...
      error: (err) => {
        console.error('[ReportingLineDefinitionList]', err);
  ...
  private updateTranslations() {
    const p = 'company.reportingLineDefinition.column.';
    this.headerActions = [
    ...
    this.columns = [
      { field: 'definitionId', headerText: this.translate.instant(p + 'id'), width: '140px', isPrimaryKey: true },
  ...
  onDelete(row: any) {
    if (!row?.definitionId) { this.notificationService.showError(this.translate.instant(TRANSLATION_KEYS.COMMON.MESSAGES.ERROR.DELETE)); return; }
    const modelFields: (keyof ReportingLineDefinition)[] = ['definitionId', 'companyId', 'tdesc', 'edesc', 'editBy', 'editDate', 'editTime', 'verified'];
    const cleanRow = filterSyncfusionFields<ReportingLineDefinition>(row, modelFields);
