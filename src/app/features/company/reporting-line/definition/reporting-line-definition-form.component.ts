...
@Component({
  selector: 'app-reporting-line-definition-form',
  standalone: true,
  ...
  templateUrl: './reporting-line-definition-form.component.html'
})
export class ReportingLineDefinitionFormComponent implements OnChanges {
  @Input() isOpen = false;
  @Input() data: ReportingLineDefinition | null = null;
  @Output() close = new EventEmitter<void>();
  @Output() save = new EventEmitter<void>();

  private fb = inject(FormBuilder);
  private service = inject(ReportingLineDefinitionService);
  ...
  constructor() {
    this.form = this.fb.group({
      definitionId: ['', [Validators.required, Validators.maxLength(20)]],
  ...
  ngOnChanges() {
    if (this.isOpen) {
      ...
      if (this.data) {
        ...
        this.form.get('definitionId')?.disable();
      } else {
        this.form.reset();
        this.form.get('definitionId')?.enable();
      }
    }
  }
  ...
  private saveData() {
    ...
    const req$ = this.isEditMode ? this.service.update(formData.definitionId, formData) : this.service.create(formData);
    req$.subscribe({
      next: () => {
        setTimeout(() => {
