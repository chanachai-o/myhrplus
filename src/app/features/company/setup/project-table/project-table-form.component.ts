...
@Component({
  selector: 'app-project-table-form',
  standalone: true,
  ...
  templateUrl: './project-table-form.component.html'
})
export class ProjectTableFormComponent implements OnChanges {
  @Input() isOpen = false;
  ...
  constructor() {
    this.form = this.fb.group({
      projectId: ['', [Validators.required, Validators.maxLength(20)]],
  ...
  private saveData() {
    ...
    const req$ = this.isEditMode ? this.service.update(formData.projectId, formData) : this.service.create(formData);
    req$.subscribe({
      next: () => {
        setTimeout(() => {
