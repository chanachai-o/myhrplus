...
import { ProjectTableFormComponent } from './project-table-form.component';
...
@Component({
  ...
  imports: [
    ...
    ProjectTableFormComponent, FormsModule, ReactiveFormsModule, EmptyStateComponent
  ],
...
export class ProjectTableListComponent implements OnInit {
  public service = inject(ProjectService);
  ...
  loadData() {
    this.service.getAll().subscribe({
      ...
      error: (err) => {
        console.error('[ProjectTableList]', err);
  ...
  private updateTranslations() {
    const p = 'company.projectTable.column.';
    this.headerActions = [
    ...
    this.columns = [
      { field: 'projectId', headerText: this.translate.instant(p + 'id'), width: '140px', isPrimaryKey: true },
  ...
  onDelete(row: any) {
    if (!row?.projectId) { this.notificationService.showError(this.translate.instant(TRANSLATION_KEYS.COMMON.MESSAGES.ERROR.DELETE)); return; }
    const modelFields: (keyof Project)[] = ['projectId', 'companyId', 'tdesc', 'edesc', 'editBy', 'editDate', 'editTime', 'verified'];
    const cleanRow = filterSyncfusionFields<Project>(row, modelFields);
