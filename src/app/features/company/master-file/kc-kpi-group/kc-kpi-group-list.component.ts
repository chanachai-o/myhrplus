...
import { KcKpiGroupService } from '../../services/kc-kpi-group.service';
import { KcKpiGroup } from '../../models/kc-kpi-group.model';
import { KcKpiGroupFormComponent } from './kc-kpi-group-form.component';
...
@Component({
  selector: 'app-kc-kpi-group-list',
  standalone: true,
  imports: [
    ...
    KcKpiGroupFormComponent, FormsModule, ReactiveFormsModule, EmptyStateComponent
  ],
  templateUrl: './kc-kpi-group-list.component.html',
  styles: [`:host { display: flex; flex-direction: column; height: 100%; gap: 1.5rem; }`]
})
export class KcKpiGroupListComponent implements OnInit {
  public service = inject(KcKpiGroupService);
  ...
  data = signal<any[]>([]);
  showModal = false;
  selectedItem: KcKpiGroup | null = null;
  ...
  loadData() {
    this.service.getAll().subscribe({
      ...
      error: (err) => {
        console.error('[KcKpiGroupList]', err);
  ...
  private updateTranslations() {
    const p = 'company.kcKpiGroup.column.';
    this.headerActions = [
    ...
    this.columns = [
      { field: 'kcKpiGroupId', headerText: this.translate.instant(p + 'id'), width: '140px', isPrimaryKey: true },
  ...
  onDelete(row: any) {
    if (!row?.kcKpiGroupId) { this.notificationService.showError(this.translate.instant(TRANSLATION_KEYS.COMMON.MESSAGES.ERROR.DELETE)); return; }
    const modelFields: (keyof KcKpiGroup)[] = ['kcKpiGroupId', 'companyId', 'tdesc', 'edesc', 'editBy', 'editDate', 'editTime', 'verified'];
    const cleanRow = filterSyncfusionFields<KcKpiGroup>(row, modelFields);
