...
@Component({
  selector: 'app-kc-kpi-group-form',
  standalone: true,
  ...
  templateUrl: './kc-kpi-group-form.component.html'
})
export class KcKpiGroupFormComponent implements OnChanges {
  @Input() isOpen = false;
  @Input() data: KcKpiGroup | null = null;
  @Output() close = new EventEmitter<void>();
  @Output() save = new EventEmitter<void>();

  private fb = inject(FormBuilder);
  private service = inject(KcKpiGroupService);
  ...
  constructor() {
    this.form = this.fb.group({
      kckpigroupid: ['', [Validators.required, Validators.maxLength(20)]],
  ...
  ngOnChanges() {
    if (this.isOpen) {
      ...
      if (this.data) {
        ...
        this.form.get('kcKpiGroupId')?.disable();
      } else {
        this.form.reset();
        this.form.get('kckpigroupid')?.enable();
      }
    }
  }
  ...
  private saveData() {
    ...
    const req$ = this.isEditMode ? this.service.update(formData.kcKpiGroupId, formData) : this.service.create(formData);
    req$.subscribe({
      next: () => {
        setTimeout(() => {
