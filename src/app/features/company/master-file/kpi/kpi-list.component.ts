...
@Component({
  ...
  imports: [
    ...
    KpiFormComponent, FormsModule, ReactiveFormsModule, EmptyStateComponent
  ],
...
export class KpiListComponent implements OnInit {
  public service = inject(KpiService);
  ...
  private updateTranslations() {
    ...
    this.columns = [
      { field: 'kpiId', headerText: this.translate.instant(p + 'id'), width: '140px', isPrimaryKey: true },
  ...
  onDelete(row: any) {
    if (!row?.kpiId) { this.notificationService.showError(this.translate.instant(TRANSLATION_KEYS.COMMON.MESSAGES.ERROR.DELETE)); return; }
    const modelFields: (keyof Kpi)[] = ['kpiId', 'companyId', 'tdesc', 'edesc', 'editBy', 'editDate', 'editTime', 'verified'];
    const cleanRow = filterSyncfusionFields<Kpi>(row, modelFields);
