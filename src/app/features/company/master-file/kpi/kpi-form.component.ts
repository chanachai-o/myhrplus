...
export class KpiFormComponent implements OnChanges {
  @Input() isOpen = false;
  @Input() data: Kpi | null = null;
  @Output() close = new EventEmitter<void>();
  @Output() save = new EventEmitter<void>();

  private fb = inject(FormBuilder);
  private service = inject(KpiService);
  ...
  constructor() {
    this.form = this.fb.group({
      kpiId: ['', [Validators.required, Validators.maxLength(20)]],
  ...
  private saveData() {
    ...
    const req$ = this.isEditMode ? this.service.update(formData.kpiId, formData) : this.service.create(formData);
    req$.subscribe({
      next: () => {
        setTimeout(() => {
