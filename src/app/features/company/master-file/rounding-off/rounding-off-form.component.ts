...
@Component({
  selector: 'app-rounding-off-form',
  standalone: true,
  ...
  templateUrl: './rounding-off-form.component.html'
})
export class RoundingOffFormComponent implements OnChanges {
  @Input() isOpen = false;
  @Input() data: RoundingOff | null = null;
  @Output() close = new EventEmitter<void>();
  @Output() save = new EventEmitter<void>();

  private fb = inject(FormBuilder);
  private service = inject(RoundingOffService);
  ...
  constructor() {
    this.form = this.fb.group({
      roundingId: ['', [Validators.required, Validators.maxLength(20)]],
  ...
  ngOnChanges() {
    if (this.isOpen) {
      ...
      if (this.data) {
        ...
        this.form.get('roundingId')?.disable();
      } else {
        this.form.reset();
        this.form.get('roundingid')?.enable();
      }
    }
  }
  ...
  private saveData() {
    ...
    const req$ = this.isEditMode ? this.service.update(formData.roundingId, formData) : this.service.create(formData);
    req$.subscribe({
      next: () => {
        setTimeout(() => {
