...
import { RoundingOffService } from '../../services/rounding-off.service';
import { RoundingOff } from '../../models/rounding-off.model';
import { RoundingOffFormComponent } from './rounding-off-form.component';
...
@Component({
  selector: 'app-rounding-off-list',
  standalone: true,
  imports: [
    ...
    RoundingOffFormComponent, FormsModule, ReactiveFormsModule, EmptyStateComponent
  ],
  templateUrl: './rounding-off-list.component.html',
  styles: [`:host { display: flex; flex-direction: column; height: 100%; gap: 1.5rem; }`]
})
export class RoundingOffListComponent implements OnInit {
  public service = inject(RoundingOffService);
  ...
  data = signal<any[]>([]);
  showModal = false;
  selectedItem: RoundingOff | null = null;
  ...
  loadData() {
    this.service.getAll().subscribe({
      ...
      error: (err) => {
        console.error('[RoundingOffList]', err);
  ...
  private updateTranslations() {
    const p = 'company.roundingOff.column.';
    this.headerActions = [
    ...
    this.columns = [
      { field: 'roundingId', headerText: this.translate.instant(p + 'id'), width: '140px', isPrimaryKey: true },
  ...
  onDelete(row: any) {
    if (!row?.roundingId) { this.notificationService.showError(this.translate.instant(TRANSLATION_KEYS.COMMON.MESSAGES.ERROR.DELETE)); return; }
    const modelFields: (keyof RoundingOff)[] = ['roundingId', 'companyId', 'tdesc', 'edesc', 'editBy', 'editDate', 'editTime', 'verified'];
    const cleanRow = filterSyncfusionFields<RoundingOff>(row, modelFields);
