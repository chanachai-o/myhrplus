...
@Component({
  selector: 'app-key-competency-form',
  standalone: true,
  ...
  templateUrl: './key-competency-form.component.html'
})
export class KeyCompetencyFormComponent implements OnChanges {
  @Input() isOpen = false;
  @Input() data: KeyCompetency | null = null;
  @Output() close = new EventEmitter<void>();
  @Output() save = new EventEmitter<void>();

  private fb = inject(FormBuilder);
  private service = inject(KeyCompetencyService);
  ...
  constructor() {
    this.form = this.fb.group({
      keyCompetencyId: ['', [Validators.required, Validators.maxLength(20)]],
  ...
  ngOnChanges() {
    if (this.isOpen) {
      ...
      if (this.data) {
        ...
        this.form.get('keyCompetencyId')?.disable();
      } else {
        this.form.reset();
        this.form.get('keyCompetencyId')?.enable();
      }
    }
  }
  ...
  private saveData() {
    ...
    const req$ = this.isEditMode ? this.service.update(formData.keyCompetencyId, formData) : this.service.create(formData);
    req$.subscribe({
      next: () => {
        setTimeout(() => {
