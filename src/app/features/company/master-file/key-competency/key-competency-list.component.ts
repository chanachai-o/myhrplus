...
import { KeyCompetencyService } from '../../services/key-competency.service';
import { KeyCompetency } from '../../models/key-competency.model';
import { KeyCompetencyFormComponent } from './key-competency-form.component';
...
@Component({
  selector: 'app-key-competency-list',
  standalone: true,
  imports: [
    ...
    KeyCompetencyFormComponent, FormsModule, ReactiveFormsModule, EmptyStateComponent
  ],
  templateUrl: './key-competency-list.component.html',
  styles: [`:host { display: flex; flex-direction: column; height: 100%; gap: 1.5rem; }`]
})
export class KeyCompetencyListComponent implements OnInit {
  public service = inject(KeyCompetencyService);
  ...
  data = signal<any[]>([]);
  showModal = false;
  selectedItem: KeyCompetency | null = null;
  ...
  loadData() {
    this.service.getAll().subscribe({
      ...
      error: (err) => {
        console.error('[KeyCompetencyList]', err);
  ...
  private updateTranslations() {
    const p = 'company.keyCompetency.column.';
    this.headerActions = [
    ...
    this.columns = [
      { field: 'keyCompetencyId', headerText: this.translate.instant(p + 'id'), width: '140px', isPrimaryKey: true },
  ...
  onDelete(row: any) {
    if (!row?.keyCompetencyId) { this.notificationService.showError(this.translate.instant(TRANSLATION_KEYS.COMMON.MESSAGES.ERROR.DELETE)); return; }
    const modelFields: (keyof KeyCompetency)[] = ['keyCompetencyId', 'companyId', 'tdesc', 'edesc', 'editBy', 'editDate', 'editTime', 'verified'];
    const cleanRow = filterSyncfusionFields<KeyCompetency>(row, modelFields);
