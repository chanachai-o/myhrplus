...
import { EPayslipSignatureService } from '../../services/e-payslip-signature.service';
import { EPayslipSignature } from '../../models/e-payslip-signature.model';
import { EPayslipSignatureFormComponent } from './e-payslip-signature-form.component';
...
@Component({
  selector: 'app-e-payslip-signature-list',
  standalone: true,
  imports: [
    ...
    EPayslipSignatureFormComponent, FormsModule, ReactiveFormsModule, EmptyStateComponent
  ],
  templateUrl: './e-payslip-signature-list.component.html',
  styles: [`:host { display: flex; flex-direction: column; height: 100%; gap: 1.5rem; }`]
})
export class EPayslipSignatureListComponent implements OnInit {
  public service = inject(EPayslipSignatureService);
  ...
  data = signal<any[]>([]);
  showModal = false;
  selectedItem: EPayslipSignature | null = null;
  ...
  loadData() {
    this.service.getAll().subscribe({
      ...
      error: (err) => {
        console.error('[EPayslipSignatureList]', err);
  ...
  private updateTranslations() {
    const p = 'company.ePayslipSignature.column.';
    this.headerActions = [
    ...
    this.columns = [
      { field: 'signatureId', headerText: this.translate.instant(p + 'id'), width: '140px', isPrimaryKey: true },
  ...
  onDelete(row: any) {
    if (!row?.signatureId) { this.notificationService.showError(this.translate.instant(TRANSLATION_KEYS.COMMON.MESSAGES.ERROR.DELETE)); return; }
    const modelFields: (keyof EPayslipSignature)[] = ['signatureId', 'companyId', 'tdesc', 'edesc', 'editBy', 'editDate', 'editTime', 'verified'];
    const cleanRow = filterSyncfusionFields<EPayslipSignature>(row, modelFields);
