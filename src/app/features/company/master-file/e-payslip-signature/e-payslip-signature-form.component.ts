...
@Component({
  selector: 'app-e-payslip-signature-form',
  standalone: true,
  ...
  templateUrl: './e-payslip-signature-form.component.html'
})
export class EPayslipSignatureFormComponent implements OnChanges {
  @Input() isOpen = false;
  @Input() data: EPayslipSignature | null = null;
  @Output() close = new EventEmitter<void>();
  @Output() save = new EventEmitter<void>();

  private fb = inject(FormBuilder);
  private service = inject(EPayslipSignatureService);
  ...
  constructor() {
    this.form = this.fb.group({
      signatureId: ['', [Validators.required, Validators.maxLength(20)]],
  ...
  ngOnChanges() {
    if (this.isOpen) {
      ...
      if (this.data) {
        ...
        this.form.get('signatureId')?.disable();
      } else {
        this.form.reset();
        this.form.get('signatureId')?.enable();
      }
    }
  }
  ...
  private saveData() {
    ...
    const req$ = this.isEditMode ? this.service.update(formData.signatureId, formData) : this.service.create(formData);
    req$.subscribe({
      next: () => {
        setTimeout(() => {
