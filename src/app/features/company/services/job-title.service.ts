...
export class JobTitleService extends BaseApiService<JobTitle> {
  protected baseUrl = 'hr/company/job-titles';
  loading = signal<boolean>(false);

  private fromApi(item: any): JobTitle {
    const camel = snakeToCamelKeys(item) as any;
    return {
      ...camel,
      jobTitleId: item.job_title_id ?? item.jobtitleid ?? camel.jobTitleId,
  ...
  private toApi(data: Partial<JobTitle>): any {
    return camelToSnakeKeys(data);
  }

  override getAll(params?: Record<string, unknown>): Observable<JobTitle[]> {
  ...
  override create(data: Partial<JobTitle>): Observable<JobTitle> {
    return this.http.post<any>(this.apiUrl, this.toApi(data)).pipe(
      map(res => this.fromApi(res))
    );
  }

  override update(id: string | number, data: Partial<JobTitle>): Observable<JobTitle> {
    return this.http.put<any>(`${this.apiUrl}/${id}`, this.toApi(data)).pipe(
      map(res => this.fromApi(res))
    );
  }

  override delete(data: Partial<JobTitle>): Observable<void> {
