...
export class KeyCompetencyService extends BaseApiService<KeyCompetency> {
  protected baseUrl = 'hr/company/key-competency';
  loading = signal<boolean>(false);

  private fromApi(item: any): KeyCompetency {
    const camel = snakeToCamelKeys(item) as any;
    return {
      ...camel,
      keyCompetencyId: item.key_competency_id ?? item.kcid ?? camel.keyCompetencyId,
  ...
  private toApi(data: Partial<KeyCompetency>): any {
    return camelToSnakeKeys(data);
  }

  override getAll(params?: Record<string, unknown>): Observable<KeyCompetency[]> {
  ...
  override create(data: Partial<KeyCompetency>): Observable<KeyCompetency> {
    return this.http.post<any>(this.apiUrl, this.toApi(data)).pipe(
      map(res => this.fromApi(res))
    );
  }

  override update(id: string | number, data: Partial<KeyCompetency>): Observable<KeyCompetency> {
    return this.http.put<any>(`${this.apiUrl}/${id}`, this.toApi(data)).pipe(
      map(res => this.fromApi(res))
    );
  }

  override delete(data: Partial<KeyCompetency>): Observable<void> {
