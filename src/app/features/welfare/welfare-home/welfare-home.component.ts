...
export class WelfareHomeComponent implements OnInit, OnDestroy {
  ...
  readonly DASHBOARD_ID = 'welfare-dashboard';

  // Dashboard Customization
  showCustomizationMenu = false;
  dashboardSections = [
    { id: 'statistics', label: 'welfare.dashboard.sections.statistics', visible: true },
    { id: 'charts', label: 'welfare.dashboard.sections.charts', visible: true },
    { id: 'recentActivities', label: 'welfare.dashboard.sections.recentActivities', visible: true },
    { id: 'pendingTasks', label: 'welfare.dashboard.sections.pendingTasks', visible: true }
  ];
  ...
  recentActivities = [
    {
      title: 'อนุมัติการลงทะเบียนสวัสดิการ: 15 รายการ',
      time: '1 ชั่วโมงที่แล้ว',
      icon: 'check_circle'
    },
    {
      title: 'เพิ่มสวัสดิการใหม่: ประกันสุขภาพครอบครัว',
      time: '3 ชั่วโมงที่แล้ว',
      icon: 'favorite'
    },
    {
      title: 'อัพเดทสวัสดิการ: กองทุนสำรอง',
      time: '1 วันที่แล้ว',
      icon: 'edit'
    },
    {
      title: 'อนุมัติการใช้งานสวัสดิการ: 8 รายการ',
      time: '2 วันที่แล้ว',
      icon: 'done_all'
    }
  ];
  ...
  pendingTasks = [
    ...
    {
      title: 'รอตรวจสอบสวัสดิการ',
      count: 3,
      icon: 'verified',
      route: '/welfare'
    },
    {
      title: 'รออัพเดทข้อมูล',
      count: 2,
      icon: 'update',
      route: '/welfare'
    }
  ...
  loadComparisonData(): void {
    ...
    if (this.comparisonPeriod === 'previous') {
      ...
      this.previousPeriodStatistics = {
        totalWelfares: this.statistics.totalWelfares.value - this.statistics.totalWelfares.change,
        enrolled: this.statistics.enrolled.value - this.statistics.enrolled.change,
        active: this.statistics.active.value - this.statistics.active.change,
      ...
      this.previousPeriodStatistics = {
        totalWelfares: Math.round(this.statistics.totalWelfares.value * 0.88),
        enrolled: Math.round(this.statistics.enrolled.value * 0.92),
        active: Math.round(this.statistics.active.value * 0.91),
        pending: Math.round(this.statistics.pending.value * 0.90),
        completed: Math.round(this.statistics.completed.value * 0.91)
      };
    }
  }
  ...
  private setBreadcrumbs(): void {
    const breadcrumbs: BreadcrumbItem[] = [
      {
        label: this.translate.instant('welfare.dashboard.breadcrumb.home') || 'Home',
        route: '/home',
        icon: 'home'
      },
      {
        label: this.translate.instant('welfare.dashboard.breadcrumb.welfare') || 'Welfare',
        route: '/welfare',
        icon: 'favorite'
      },
      {
        label: this.translate.instant('welfare.dashboard.breadcrumb.dashboard') || 'Dashboard',
        icon: 'dashboard'
      }
    ];
  ...
  private getPrimaryColor(): string {
    ...
    return primaryColor || '#ec4899'; // Fallback to pink-500
  }
  ...
  private getPrimaryColorRgb(): string {
    ...
    return primaryRgb || '236, 72, 153'; // Fallback to pink-500 RGB
  }
  ...
  private getPrimaryColorHex(): string {
    const root = document.documentElement;
    const primaryColor = getComputedStyle(root).getPropertyValue('--primary-color').trim();
    return primaryColor || '#ec4899'; // Fallback
  }
  ...
  private getPrimaryColorRgbArray(): [number, number, number] {
    ...
    return [236, 72, 153]; // Fallback to pink-500
  }
  ...
  initializeCharts(): void {
    // Welfare Enrollment Chart (Last 6 months) - Colorful bars
    ...
    // Welfare Distribution Chart (Pie/Donut Chart) - Diverse colors
    ...
    this.distributionChartOption = {
      ...
      series: [{
        ...
        data: welfareTypes.map((name, index) => ({
          name,
          value: welfareData[index],
          itemStyle: {
    ...
    // Welfare by Department Chart (Stacked Bar Chart) - Gradient colors
          ...
          itemStyle: {
            color: {
              ...
              colorStops: [
                ...
                { offset: 1, color: 'rgba(244, 63, 94, 0.7)' }
              ]
            },
    ...
    // Welfare Usage Chart (Area Chart) - Gradient with primary color
  ...
  exportCharts(format: 'pdf' | 'excel'): void {
    ...
    setTimeout(() => {
      ...
      const message = this.translate.instant('welfare.dashboard.export.success', { format: formatName }) || `ส่งออกกราฟเป็น ${formatName} เรียบร้อยแล้ว`;
  ...
  exportChart(chartType: string, format: 'pdf' | 'excel'): void {
    ...
    setTimeout(() => {
      this.isExporting.set(false);
      const message = this.translate.instant('welfare.dashboard.export.chartSuccess', {
        chart: chartType,
        format: formatName
  ...
  filterActivities(): void {
    ...
    const message = this.translate.instant('welfare.dashboard.activities.filterComingSoon') || 'Filter feature coming soon';
